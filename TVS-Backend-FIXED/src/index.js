...
// Universal email sender: tries Resend API first, then SMTP/nodemailer
async function sendEmail({ to, subject, html }) {
  ...
  if (resendKey) {
    try {
      ...
      const resp = await axios.post('https://api.resend.com/emails', { from, to: Array.isArray(to) ? to : [to], subject, html },
      ...
      console.error('Resend error:', resp.data);
      return { success: false, provider: 'resend', error: JSON.stringify(resp.data) };
    } catch(e) {
      console.error('Resend API error:', e.response?.data || e.message);
