...
app.get("/api/analytics/all-campaigns", auth, async (req, res) => {
  try {
    ...
    const salesData = await pool.query(`SELECT COALESCE(utm_campaign,'unknown') as campaign, COUNT(*) as orders, COALESCE(SUM(revenue),0) as revenue FROM orders WHERE utm_medium='email' AND order_date::date BETWEEN $1 AND $2 GROUP BY utm_campaign`,[start,end]);
    ...
    const salesMap = {};
    for (const r of salesData.rows) salesMap[r.campaign] = r;
    const syncedMap = {};
    ...
    const merged = campaigns.map(camp => {
      ...
      const s = salesMap[camp.id] || {};
      // Try to match synced stats by partial name match
      let synced = syncedMap[camp.id];
      if (!synced) {
        for (const [k,v] of Object.entries(syncedMap)) {
          if (k.includes(camp.id) || camp.id.includes(k) || (v.campaign_name && camp.name.toLowerCase().includes(v.campaign_name.toLowerCase().slice(0,15)))) {
            synced = v; break;
          }
        }
      }
    ...
    res.json({ campaigns: merged, groups, total: merged.length });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get("/api/finance/balances", auth, async (req, res) => {
  try {
    const results = { paypal: null, elavon: null };
