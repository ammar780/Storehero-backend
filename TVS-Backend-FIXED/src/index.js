...
app.get("/api/analytics/email-performance", auth, async (req, res) => {
  try {
    ...
    // Opens from tracking pixels
    const opens = await pool.query(`SELECT campaign, email_id, COUNT(*) as total_opens, COUNT(DISTINCT recipient) as unique_opens FROM email_tracking WHERE event_type='open' AND tracked_at::date BETWEEN $1 AND $2 GROUP BY campaign, email_id ORDER BY total_opens DESC`,[start,end]);
    
    // Sales from UTM tracking (orders with utm_campaign set)
    const sales = await pool.query(`SELECT COALESCE(utm_source,'unknown') as source, COALESCE(SPLIT_PART(utm_medium,'_',1),'unknown') as medium, COALESCE(utm_source,'') || ':' || COALESCE(utm_medium,'') as channel, COUNT(*) as orders, COALESCE(SUM(revenue),0) as revenue, COALESCE(SUM(gross_profit),0) as profit, CASE WHEN COUNT(*)>0 THEN SUM(revenue)/COUNT(*) ELSE 0 END as aov FROM orders WHERE utm_medium='email' AND order_date::date BETWEEN $1 AND $2 GROUP BY utm_source, utm_medium ORDER BY revenue DESC`,[start,end]);
    
    // Sales by campaign (utm_campaign)
    const byCampaign = await pool.query(`SELECT COALESCE(utm_campaign,'unknown') as campaign, COALESCE(utm_source,'unknown') as provider, COUNT(*) as orders, COALESCE(SUM(revenue),0) as revenue, COALESCE(SUM(gross_profit),0) as profit, CASE WHEN COUNT(*)>0 THEN SUM(revenue)/COUNT(*) ELSE 0 END as aov FROM orders WHERE utm_medium='email' AND order_date::date BETWEEN $1 AND $2 GROUP BY utm_campaign, utm_source ORDER BY revenue DESC`,[start,end]);
    
    // Sales by specific email (utm_content)  
    const byEmail = await pool.query(`SELECT COALESCE(utm_campaign,'unknown') as campaign, COALESCE(utm_content,'unknown') as email_cta, COALESCE(utm_source,'unknown') as provider, COUNT(*) as orders, COALESCE(SUM(revenue),0) as revenue, COALESCE(SUM(gross_profit),0) as profit, CASE WHEN COUNT(*)>0 THEN SUM(revenue)/COUNT(*) ELSE 0 END as aov FROM orders WHERE utm_medium='email' AND order_date::date BETWEEN $1 AND $2 GROUP BY utm_campaign, utm_content, utm_source ORDER BY revenue DESC LIMIT 100`,[start,end]);
    ...
    // Total opens
    const totalOpens = await pool.query(`SELECT COUNT(*) as total_opens, COUNT(DISTINCT recipient) as unique_openers, COUNT(DISTINCT campaign) as campaigns_tracked FROM email_tracking WHERE event_type='open' AND tracked_at::date BETWEEN $1 AND $2`,[start,end]);
    ...
    // Opens by campaign
    const opensByCampaign = await pool.query(`SELECT campaign, COUNT(*) as opens, COUNT(DISTINCT recipient) as unique_opens, COUNT(DISTINCT email_id) as emails_tracked FROM email_tracking WHERE event_type='open' AND tracked_at::date BETWEEN $1 AND $2 GROUP BY campaign ORDER BY opens DESC`,[start,end]);
    
    res.json({
      summary: { ...summary.rows[0], ...totalOpens.rows[0] },
      byProvider: sales.rows,
      byCampaign: byCampaign.rows,
      byEmail: byEmail.rows,
      opens: opens.rows,
      opensByCampaign: opensByCampaign.rows,
      daily: daily.rows,
    });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
(async () => {
  const migrations = [
    ...
    "CREATE TABLE IF NOT EXISTS email_tracking(id SERIAL PRIMARY KEY,campaign VARCHAR(255),email_id VARCHAR(255),recipient VARCHAR(500),event_type VARCHAR(50) DEFAULT 'open',tracked_at TIMESTAMPTZ DEFAULT NOW())",
