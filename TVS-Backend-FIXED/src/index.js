...
app.get('/health',(r,s)=>s.json({status:'healthy',app:APP_NAME,ts:new Date().toISOString()}));
...
app.post('/api/auth/login',async(req,res)=>{try{
  ...
  if (!otp) {
    ...
    if (process.env.SMTP_HOST && process.env.SMTP_USER) {
      ...
      } catch(smtpErr) { console.error('OTP email failed:', smtpErr.message); }
    }
...
app.post('/api/auth/reset-password',async(req,res)=>{try{
  ...
  if(!otp){// Step 1: Send OTP
    ...
    if(process.env.SMTP_HOST&&process.env.SMTP_USER){try{const t=nodemailer.createTransport({host:process.env.SMTP_HOST,port:+(process.env.SMTP_PORT)||587,secure:false,auth:{user:process.env.SMTP_USER,pass:process.env.SMTP_PASS},tls:{rejectUnauthorized:false}});await t.sendMail({from:process.env.EMAIL_FROM||process.env.SMTP_USER,to:email,subject:`${APP_NAME} — Reset Code: ${code}`,html:`<div style="text-align:center;font-family:Arial"><h2 style="color:#f1c349">Password Reset</h2><div style="font-size:36px;font-weight:bold;letter-spacing:8px;margin:24px 0;padding:16px;background:#f5f5f5;border-radius:12px">${code}</div><p style="color:#999;font-size:12px">Expires in 5 minutes.</p></div>`});}catch(e){console.error('Reset OTP failed:',e.message);}}
...
app.post('/api/sync/rebuild-metrics',auth,async(req,res)=>{try{
  ...
  // Apply fixed costs
  const fc=await pool.query('SELECT COALESCE(SUM(amount_monthly),0)as t FROM fixed_costs WHERE is_active=true');
  await pool.query(`UPDATE daily_metrics SET fixed_costs_daily=$1,contribution_margin=gross_profit-ad_spend,net_profit=gross_profit-ad_spend-$1-COALESCE(affiliate_commissions,0)-COALESCE(store_credit_used,0),mer=CASE WHEN ad_spend>0 THEN revenue/ad_spend ELSE 0 END WHERE date>=date_trunc('month',CURRENT_DATE)`,[+(fc.rows[0].t)/daysInCurrentMonth()]);
  ...
  if (!platform || platform === "google") {
    ...
    if (gDevToken && gRefresh && gClientId) {
      try {
        ...
        await pool.query(`UPDATE daily_metrics dm SET google_spend=COALESCE(sub.spend,0), ad_spend=COALESCE(dm.meta_spend,0)+COALESCE(sub.spend,0)+COALESCE(dm.tiktok_spend,0)+COALESCE(dm.microsoft_spend,0) FROM (SELECT date,SUM(spend) as spend FROM ad_spend_daily WHERE platform='google' GROUP BY date) sub WHERE dm.date=sub.date`);
  ...
  if (!platform || platform === "tiktok") {
    ...
    if (tToken && tAdvId) {
      try {
        ...
        await pool.query(`UPDATE daily_metrics dm SET tiktok_spend=COALESCE(sub.spend,0), ad_spend=COALESCE(dm.meta_spend,0)+COALESCE(dm.google_spend,0)+COALESCE(sub.spend,0)+COALESCE(dm.microsoft_spend,0) FROM (SELECT date,SUM(spend) as spend FROM ad_spend_daily WHERE platform='tiktok' GROUP BY date) sub WHERE dm.date=sub.date`);
...
app.post("/api/sync/referly", auth, async (req, res) => {
  try {
    ...
    try {
      ...
      await pool.query(`UPDATE daily_metrics SET net_profit=gross_profit-ad_spend-fixed_costs_daily-COALESCE(affiliate_commissions,0)-COALESCE(store_credit_used,0)
...
app.get("/api/dashboard/waterfall", auth, async (req, res) => {
  try {
    const { start, end } = dr(req.query);
    const r = await pool.query(`SELECT COALESCE(SUM(revenue),0) as revenue,COALESCE(SUM(cogs),0) as cogs,COALESCE(SUM(shipping_cost),0) as shipping,COALESCE(SUM(payment_fees),0) as fees,COALESCE(SUM(discount_total),0) as discounts,COALESCE(SUM(refund_total),0) as refunds,COALESCE(SUM(ad_spend),0) as ad_spend,COALESCE(SUM(fixed_costs_daily),0) as fixed_costs,COALESCE(SUM(gross_profit),0) as gross_profit,COALESCE(SUM(net_profit),0) as net_profit,COALESCE(SUM(store_credit_used),0) as store_credits,COALESCE(SUM(affiliate_commissions),0) as affiliate_commissions FROM daily_metrics WHERE date BETWEEN $1 AND $2`, [start, end]);
    const d = r.rows[0];
    const waterfall = [
...
async function sendWeeklyReport() {
  try {
    ...
    const transporter = nodemailer.createTransport({ host: process.env.SMTP_HOST, port: +(process.env.SMTP_PORT)||587, secure: false, auth: { user: process.env.SMTP_USER, pass: process.env.SMTP_PASS }, tls: { rejectUnauthorized: false } });
    const info = await transporter.sendMail({ from: process.env.EMAIL_FROM || process.env.SMTP_USER, to: recipients.join(','), subject: `Vitamin Shots Weekly Report — Revenue: ${fc(tw.revenue)} | Profit: ${fc(tw.net_profit)}`, html });
...
cron.schedule('0 3 * * *',async()=>{console.log('⏰ Daily metrics rebuild');try{const fc=await pool.query('SELECT COALESCE(SUM(amount_monthly),0)as t FROM fixed_costs WHERE is_active=true');await pool.query(`UPDATE daily_metrics SET fixed_costs_daily=$1,contribution_margin=gross_profit-ad_spend,net_profit=gross_profit-ad_spend-$1-COALESCE(affiliate_commissions,0)-COALESCE(store_credit_used,0),mer=CASE WHEN ad_spend>0 THEN revenue/ad_spend ELSE 0 END WHERE date>=date_trunc('month',CURRENT_DATE)`,[+(fc.rows[0].t)/daysInCurrentMonth()]);console.log('✅ Daily rebuild done')}catch(e){console.error(e.message)}},{timezone:cronTz});
...
(async () => {
  const migrations = [
    ...
    "INSERT INTO integrations(platform,is_connected) VALUES('microsoft_ads',false),('google_analytics',false),('search_console',false),('referly',false),('paypal',false),('slack',false) ON CONFLICT(platform) DO NOTHING",
