...
app.post("/api/sync/microsoft-ads", auth, async (req, res) => {
  try {
    ...
    if (reportResp.data?.ReportRequestId) {
      ...
      await pool.query(`UPDATE daily_metrics dm SET microsoft_spend=COALESCE(sub.spend,0), ad_spend=COALESCE(dm.meta_spend,0)+COALESCE(dm.google_spend,0)+COALESCE(dm.tiktok_spend,0)+COALESCE(sub.spend,0) FROM (SELECT date,SUM(spend) as spend FROM ad_spend_daily WHERE platform='microsoft' GROUP BY date) sub WHERE dm.date=sub.date`);
...
app.post("/api/inventory/ai-alert", auth, async (req, res) => {
  try {
    ...
    const { data: inv } = await axios.get(`http://localhost:${PORT}/api/inventory/forecast`, { headers: { Authorization: req.headers.authorization } });
    const critical = inv.forecasts.filter(f=>['out_of_stock','critical','reorder'].includes(f.status));
    ...
    sendSlack('📦 Stock alert: ' + critical.length + ' products need reordering. Total cost: $' + inv.summary.totalReorderCost);
    res.json({ sent: result.success, products: critical.length, totalCost: inv.summary.totalReorderCost, aiAnalysis: aiText });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
cron.schedule('0 9 * * *', async () => {
  ...
  try {
    const lowStock = await pool.query("SELECT name, stock_quantity, avg_daily_sales, days_of_stock FROM products WHERE is_active=true AND stock_quantity>0 AND (days_of_stock<14 OR stock_quantity<reorder_point) ORDER BY days_of_stock ASC");
