...
app.put('/api/settings/integrations/:platform',auth,async(req,res)=>{try{const{config,is_connected}=req.body;
  ...
  res.json((await pool.query("INSERT INTO integrations(platform,config,is_connected,updated_at) VALUES($3,COALESCE($1,'{}'),COALESCE($2,false),NOW()) ON CONFLICT(platform) DO UPDATE SET config=COALESCE($1,integrations.config),is_connected=COALESCE($2,integrations.is_connected),updated_at=NOW() RETURNING id,platform,is_connected,last_sync_at,sync_status",[mergedConfig,is_connected,req.params.platform])).rows[0]);
}catch(e){res.status(500).json({error:e.message})}});
