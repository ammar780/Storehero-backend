...
app.post("/api/sync/google-analytics", auth, async (req, res) => {
  try {
    ...
    console.error('GA4 sync error:', e.response?.data || e.message);
...
app.post("/api/sync/search-console", auth, async (req, res) => {
  try {
    ...
    console.error('Search Console sync error:', e.response?.data || e.message);
