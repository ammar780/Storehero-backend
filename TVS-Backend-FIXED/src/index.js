...
app.post("/api/hub/weekly-digest", auth, async (req, res) => {
  try {
    ...
    for (const u of users.rows) {
      await sendEmail(u.email, '📊 Weekly Business Digest — All Products', '<div style="font-family:Arial;max-width:600px;margin:0 auto;"><h1 style="color:#f1c349;">Weekly Business Digest</h1><pre style="white-space:pre-wrap;font-size:14px;line-height:1.6;">' + digest.replace(/\n/g, '<br>') + '</pre><hr><p style="color:#888;font-size:12px;">Vitamin Shots Finance Minister · Command Hub</p></div>');
    }
    
    res.json({ sent: true, recipients: users.rows.length, digest });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
