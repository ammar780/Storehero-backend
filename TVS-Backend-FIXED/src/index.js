...
app.get("/api/analytics/email-performance", auth, async (req, res) => {
  try {
    ...
    res.json({
      summary: { ...summary.rows[0], total_sends: Math.max(+(sends.rows[0]?.total_sends||0), syncedTotalSent), unique_recipients: +(sends.rows[0]?.unique_recipients||0), total_opens: +(opens.rows[0]?.total_opens||0), unique_openers: Math.max(+(opens.rows[0]?.unique_openers||0), syncedTotalOpens), total_clicks: Math.max(+(clicks.rows[0]?.total_clicks||0), syncedTotalClicks), open_rate: +(sends.rows[0]?.total_sends||0)>0 ? Math.round(+(opens.rows[0]?.unique_openers||0)/+(sends.rows[0]?.total_sends||0)*1000)/10 : 0 },
