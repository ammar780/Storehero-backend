...
// ── Additional migrations for final build ──
const EXTRA_SQL = [
  ...
  `CREATE TABLE IF NOT EXISTS notification_log(
    id SERIAL PRIMARY KEY, type VARCHAR(100) NOT NULL, title VARCHAR(500),
    message TEXT, channel VARCHAR(50) DEFAULT 'email', sent_at TIMESTAMPTZ DEFAULT NOW(),
    metadata JSONB DEFAULT '{}')`,
  // AI insights improvements
  `ALTER TABLE ai_insights ADD COLUMN IF NOT EXISTS model VARCHAR(100) DEFAULT 'claude'`,
];
  ...
  try {
    for (const sql of EXTRA_SQL) {
      try { await c.query(sql); } catch(e) {
        if (!e.message.includes('already exists') && !e.message.includes('duplicate'))
          console.warn('Extra migration warn:', e.message.slice(0,80));
      }
    }
    console.log('Extra migrations done');
  } finally { c.release(); }
}
// Run extra after main
run = (orig => async () => { await orig(); const p2 = new Pool({connectionString:process.env.DATABASE_URL,ssl:process.env.NODE_ENV==='production'?{rejectUnauthorized:false}:false}); const c=await p2.connect(); try{for(const s of EXTRA_SQL){try{await c.query(s)}catch(e){if(!e.message.includes('already exists')&&!e.message.includes('duplicate'))console.warn(e.message.slice(0,80))}}console.log('Extra migrations done')}finally{c.release();await p2.end()} })(run);
