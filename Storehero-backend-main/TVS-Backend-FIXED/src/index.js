...
require('express-async-errors'); // forwards async route throws to the error handler (prevents hung requests)

const app=express(),PORT=process.env.PORT||3001;
const pool=new Pool({connectionString:process.env.DATABASE_URL,ssl:process.env.NODE_ENV==='production'?{rejectUnauthorized:false}:false,max:10,idleTimeoutMillis:30000,connectionTimeoutMillis:5000,statement_timeout:30000});
...
// OTP storage (Postgres-backed so it survives restarts and works across instances)
const otpSet = async (key, code, ttlMs) => { await pool.query("INSERT INTO otp_codes(key,code,expires_at) VALUES($1,$2,$3) ON CONFLICT(key) DO UPDATE SET code=$2,expires_at=$3", [key, code, new Date(Date.now()+ttlMs)]); };
const otpGet = async (key) => { try { const r = await pool.query("SELECT code,expires_at FROM otp_codes WHERE key=$1",[key]); if(!r.rows[0]) return null; return { code: r.rows[0].code, expires: new Date(r.rows[0].expires_at).getTime() }; } catch { return null; } };
const otpDel = async (key) => { try { await pool.query("DELETE FROM otp_codes WHERE key=$1",[key]); } catch {} };
// #11+#12 Periodic cleanup of expired OTPs (DB) and the in-memory rate limiter
setInterval(()=>{const now=Date.now();for(const[k,a]of loginAttempts){const r=a.filter(t=>now-t<15*60*1000);if(!r.length)loginAttempts.delete(k);else loginAttempts.set(k,r);}pool.query("DELETE FROM otp_codes WHERE expires_at < NOW()").catch(()=>{});},60000);
...
app.post('/api/auth/login',async(req,res)=>{try{
  ...
  if (!otp) {
    ...
    await otpSet(email, code, 5 * 60 * 1000);
...
app.post('/api/auth/reset-password',async(req,res)=>{try{
  ...
  if(!otp){// Step 1: Send OTP
    ...
    const code=String(Math.floor(100000+Math.random()*900000));await otpSet('reset_'+email, code, 5*60*1000);
    ...
    if (!resetEmailResult.success) { await otpDel('reset_'+email); return res.status(500).json({ error: 'Failed to send reset code. Check email configuration.' }); }
    return res.json({otpRequired:true,message:'Reset code sent to your email'});
  }// Step 2: Verify OTP and reset
  if(!newPassword||newPassword.length<8)return res.status(400).json({error:'Password must be at least 8 characters'});
  const stored=await otpGet('reset_'+email);if(!stored||Date.now()>stored.expires)return res.status(401).json({error:'Invalid or expired reset code'});
  ...
  await otpDel('reset_'+email);const h=await bcrypt.hash(newPassword,12);
...
app.put('/api/settings/integrations/:platform',auth,async(req,res)=>{try{const{config,is_connected}=req.body;
  ...
  res.json((await pool.query("INSERT INTO integrations(platform,config,is_connected,updated_at) VALUES($3,COALESCE($1::jsonb,'{}'::jsonb),COALESCE($2,false),NOW()) ON CONFLICT(platform) DO UPDATE SET config=COALESCE($1::jsonb,integrations.config),is_connected=COALESCE($2,integrations.is_connected),updated_at=NOW() RETURNING id,platform,is_connected,last_sync_at,sync_status",[mergedConfig,is_connected,req.params.platform])).rows[0]);
}catch(e){res.status(500).json({error:e.message})}});

// --- Hub connections: catalog-driven config + live test ---
// --- Custom Plugins engine (isolated drop-in module) ---
const pluginsHub = require('./plugins-hub');
pluginsHub.mount(app, pool, auth, claudeAnalyze);

app.get('/api/hub/connections',auth,async(req,res)=>{try{
  const rows=(await pool.query("SELECT platform,config,is_connected,error_message,last_sync_at FROM integrations WHERE platform LIKE 'hub_%'")).rows;
  const map=Object.fromEntries(rows.map(r=>[r.platform,r]));
  const connections=HUB_PRODUCTS.map(p=>{
    const r=map['hub_'+p.id]||{};
    let cfg={};try{cfg=typeof r.config==='string'?JSON.parse(r.config):(r.config||{});}catch{}
    const local=p.id==='tvs_store';
    return{id:p.id,name:p.name,icon:p.icon,desc:p.desc,defaultUrl:p.defaultUrl||'',local,url:cfg.url||'',hasKey:!!cfg.api_key,connected:local?true:!!r.is_connected,error:r.error_message||null,lastTest:r.last_sync_at||null};
  });
  res.json({connections});
}catch(e){res.status(500).json({error:e.message})}});

app.post('/api/hub/connections/:id/test',auth,async(req,res)=>{try{
  const p=HUB_PRODUCTS.find(x=>x.id===req.params.id);
  if(!p)return res.status(404).json({error:'Unknown product'});
  if(p.id==='tvs_store')return res.json({connected:true,local:true,sample:[]});
  const stats=await fetchProductStats(p);
  const connected=!!stats?.connected;
  const error=connected?null:(stats?.error||'No response from app');
  await pool.query("INSERT INTO integrations(platform,is_connected,error_message,last_sync_at,updated_at) VALUES($1,$2,$3,NOW(),NOW()) ON CONFLICT(platform) DO UPDATE SET is_connected=$2,error_message=$3,last_sync_at=NOW(),updated_at=NOW()",['hub_'+p.id,connected,error]);
  const m=stats?.metrics||{};const sample=Object.keys(m).slice(0,3).map(k=>({key:k,value:m[k]}));
  res.json({connected,error,sample});
}catch(e){res.status(500).json({error:e.message})}});
...
// ── Emailit Sync (pulls from Emailit's List Emails API, grouped by meta.campaign) ──
// Complements the real-time webhook: this is a full historical pull/reconcile, same
// data model as the Enginemailer sync above so both providers feed the same table.
app.post("/api/sync/emailit", auth, syncRateLimit, async (req, res) => {
  try {
    const ec = await getCreds('emailit');
    const apiKey = ec.api_key || process.env.EMAILIT_API_KEY;
    const baseUrl = ec.api_base_url || "https://api.emailit.com/v1";
    if (!apiKey) return res.status(400).json({ error: "Emailit API key not configured. Go to Settings > Integrations > Emailit." });
    const since = req.body.date_from || new Date(Date.now() - 90 * 864e5).toISOString().split("T")[0];
    const headers = { "Authorization": "Bearer " + apiKey, "Content-Type": "application/json" };

    // campaign -> { sent, delivered, opened(unique via Set), clicked(unique via Set), bounced, complained, revenue }
    const byCampaign = new Map();
    const bump = (campaign) => {
      if (!byCampaign.has(campaign)) byCampaign.set(campaign, { sent: 0, delivered: 0, opens: new Set(), clicks: new Set(), bounced: 0, complained: 0 });
      return byCampaign.get(campaign);
    };

    let page = 1, totalPulled = 0;
    const MAX_PAGES = 50; // safety cap — 50 pages * up to 100/page covers realistic volumes
    while (page <= MAX_PAGES) {
      const resp = await axios.get(baseUrl + '/emails', {
        headers, params: { page, per_page: 100, since, type: 'campaign' }
      }).catch(e => ({ data: null, error: e.message }));
      const list = resp?.data?.data || resp?.data?.emails || resp?.data?.results || [];
      if (!list.length) break;
      totalPulled += list.length;
      for (const em of list) {
        const campaign = em?.meta?.campaign || em?.metadata?.campaign || 'unknown';
        const c = bump(campaign);
        c.sent++;
        const status = (em.status || '').toLowerCase();
        if (['delivered', 'opened', 'clicked'].includes(status)) c.delivered++;
        if (['bounced', 'soft_bounced', 'failed', 'rejected'].includes(status)) c.bounced++;
        if (status === 'complained') c.complained++;
        if (em.opened_at || em.opens > 0) c.opens.add(em.id || em.recipient);
        if (em.clicked_at || em.clicks > 0) c.clicks.add(em.id || em.recipient);
      }
      if (list.length < 100) break; // last page
      page++;
    }

    let totalSent = 0, totalOpens = 0, totalClicks = 0, seriesSynced = 0;
    for (const [campaign, c] of byCampaign.entries()) {
      const opens = c.opens.size, clicks = c.clicks.size;
      totalSent += c.sent; totalOpens += opens; totalClicks += clicks;
      const campId = 'emailit_campaign_' + campaign.toLowerCase().replace(/[^a-z0-9]+/g, '_').slice(0, 80);
      await pool.query(`INSERT INTO email_campaign_stats(campaign_id,campaign_name,provider,sent,opens,unique_opens,clicks,bounces,unsubscribes,revenue,synced_at)
        VALUES($1,$2,'emailit',$3,$4,$5,$6,$7,0,0,NOW())
        ON CONFLICT(campaign_id) DO UPDATE SET sent=$3,opens=$4,unique_opens=$5,clicks=$6,bounces=$7,synced_at=NOW()`,
        [campId, campaign, c.sent, opens, opens, clicks, c.bounced]);
      seriesSynced++;
    }

    await setIntStatus('emailit', 'synced');
    res.json({ status: "synced", campaigns: seriesSynced, emailsPulled: totalPulled, totalSent, totalOpens, totalClicks, message: `Synced ${seriesSynced} campaigns from ${totalPulled} emails` });
  } catch (e) { await setIntStatus('emailit', 'error', e.message); res.status(500).json({ error: e.message }); }
});

// ── Provider-scoped campaign detail (powers the new Emailit / Enginemailer tabs) ──
// Returns EVERY campaign for that provider, with parent/child (autoresponder series) structure,
// computed rates, and nothing filtered out — this is the "don't miss anyone" view.
async function providerCampaignDetail(provider) {
  const rows = (await pool.query(
    `SELECT campaign_id, campaign_name, parent_campaign, sent, opens, unique_opens, clicks, bounces, unsubscribes, revenue, synced_at
     FROM email_campaign_stats WHERE provider=$1 ORDER BY parent_campaign NULLS FIRST, sent DESC`, [provider]
  )).rows;
  const rate = (num, den) => den > 0 ? Math.round((num / den) * 1000) / 10 : 0;
  const withRates = rows.map(r => ({
    ...r,
    open_rate: rate(r.unique_opens, r.sent),
    click_rate: rate(r.clicks, r.sent),
    bounce_rate: rate(r.bounces, r.sent),
    click_to_open_rate: rate(r.clicks, r.unique_opens),
  }));
  // Group children under their parent series (autoresponder emails), everything else stays top-level
  const parents = withRates.filter(r => !r.parent_campaign);
  const children = withRates.filter(r => r.parent_campaign);
  const grouped = parents.map(p => ({ ...p, emails: children.filter(c => c.parent_campaign === p.campaign_id) }));
  // Orphan children (parent not synced/found) still get surfaced, never dropped
  const orphans = children.filter(c => !parents.some(p => p.campaign_id === c.parent_campaign));
  const totals = withRates.reduce((a, r) => ({ sent: a.sent + (+r.sent||0), opens: a.opens + (+r.unique_opens||0), clicks: a.clicks + (+r.clicks||0), bounces: a.bounces + (+r.bounces||0), revenue: a.revenue + (+r.revenue||0) }), { sent: 0, opens: 0, clicks: 0, bounces: 0, revenue: 0 });
  return {
    provider, totals: { ...totals, open_rate: rate(totals.opens, totals.sent), click_rate: rate(totals.clicks, totals.sent), bounce_rate: rate(totals.bounces, totals.sent) },
    campaignCount: parents.length + orphans.length, totalEmailsInSeries: rows.length,
    campaigns: [...grouped, ...orphans.map(o => ({ ...o, emails: [] }))],
  };
}
app.get('/api/emails/enginemailer', auth, async (req, res) => { try { res.json(await providerCampaignDetail('enginemailer')); } catch (e) { res.status(500).json({ error: e.message }); } });
app.get('/api/emails/emailit', auth, async (req, res) => { try { res.json(await providerCampaignDetail('emailit')); } catch (e) { res.status(500).json({ error: e.message }); } });
...
// ====================== EMAILIT WEBHOOKS ======================
// Real delivery/bounce/spam-complaint data straight from Emailit — feeds every
// plugin's sends automatically as long as they set meta.campaign on each send.
// Register this URL in the Emailit dashboard: https://<this-backend>/api/webhooks/emailit
function verifyEmailitSignature(rawBody, signature, timestamp, secret) {
  ...
  try { return crypto.timingSafeEqual(Buffer.from(computed,'hex'), Buffer.from(String(signature),'hex')); }
  catch { return false; }
}
const EMAILIT_EVENT_MAP = { 'email.delivered':'delivered', 'email.bounced':'bounced', 'email.attempted':'soft_bounce', 'email.failed':'failed', 'email.rejected':'rejected', 'email.suppressed':'suppressed', 'email.complained':'spam_complaint', 'email.clicked':'click', 'email.loaded':'open', 'email.accepted':'accepted' };
app.post('/api/webhooks/emailit', async (req, res) => {
  try {
    ...
    if (secret) {
      if (!verifyEmailitSignature(req.rawBody, signature, timestamp, secret)) {
        return res.status(401).json({ error: 'Invalid signature' });
      }
    } // if no secret configured yet, accept (so setup isn't blocked) but log a warning
    else console.warn('[emailit-webhook] EMAILIT_WEBHOOK_SECRET not set — accepting unverified request');

    const body = req.body || {};
    const type = body.type;
    const obj = body?.data?.object || {};
    const eventType = EMAILIT_EVENT_MAP[type];
    if (!eventType) return res.json({ received: true, ignored: type }); // unhandled event types (domain.*, contact.*, etc) — no-op

    // Click/load events nest the email under obj.email; status events have it flat.
    const isNested = type === 'email.clicked' || type === 'email.loaded';
    const emailObj = isNested ? (obj.email || {}) : obj;
    const campaign = emailObj?.meta?.campaign || 'unknown';
    const emailId = emailObj?.id || obj?.id || 'unknown';
    const recipient = isNested ? (emailObj?.rcpt_to || 'unknown') : (emailObj?.to || 'unknown');

    await pool.query(
      `INSERT INTO email_tracking(campaign,email_id,recipient,event_type,provider,tracked_at) VALUES($1,$2,$3,$4,'emailit',NOW())`,
      [campaign, emailId, recipient, eventType]
    );
    res.json({ received: true, event: eventType, campaign });
  } catch (e) {
    console.error('[emailit-webhook] error:', e.message);
...
app.get("/api/analytics/email-performance", auth, async (req, res) => {
  try {
    ...
    // ── By campaign: full funnel (sends + delivered + bounced + spam + opens + clicks + sales merged) ──
    const byCampaign = await pool.query(`
      SELECT c.campaign, c.sends, c.unique_recipients,
        COALESCE(d.delivered,0) as delivered, COALESCE(b.bounced,0) as bounced, COALESCE(sp.spam_complaints,0) as spam_complaints,
        COALESCE(o.opens,0) as opens, COALESCE(o.unique_openers,0) as unique_openers,
        COALESCE(cl.clicks,0) as clicks, COALESCE(cl.unique_clickers,0) as unique_clickers,
        COALESCE(s.orders,0) as orders, COALESCE(s.revenue,0) as revenue, COALESCE(s.profit,0) as profit,
        CASE WHEN c.sends>0 THEN ROUND(COALESCE(d.delivered,0)::numeric/c.sends*100,1) ELSE 0 END as delivery_rate,
        CASE WHEN c.sends>0 THEN ROUND(COALESCE(b.bounced,0)::numeric/c.sends*100,1) ELSE 0 END as bounce_rate,
        CASE WHEN c.sends>0 THEN ROUND(COALESCE(sp.spam_complaints,0)::numeric/c.sends*100,2) ELSE 0 END as spam_rate,
        CASE WHEN c.sends>0 THEN ROUND(COALESCE(o.unique_openers,0)::numeric/c.sends*100,1) ELSE 0 END as open_rate,
        CASE WHEN c.sends>0 THEN ROUND(COALESCE(cl.unique_clickers,0)::numeric/c.sends*100,1) ELSE 0 END as click_rate,
        CASE WHEN COALESCE(o.unique_openers,0)>0 THEN ROUND(COALESCE(cl.unique_clickers,0)::numeric/o.unique_openers*100,1) ELSE 0 END as click_to_open_rate,
        ...
        CASE WHEN c.sends>0 AND ROUND(COALESCE(sp.spam_complaints,0)::numeric/c.sends*100,2)>=0.3 THEN 'likely_spam'
      ...
      LEFT JOIN (SELECT campaign, COUNT(*) as delivered FROM email_tracking WHERE event_type='delivered' AND tracked_at::date BETWEEN $1 AND $2 GROUP BY campaign) d ON d.campaign=c.campaign
      LEFT JOIN (SELECT campaign, COUNT(*) as bounced FROM email_tracking WHERE event_type IN ('bounced','soft_bounce','failed','rejected','suppressed') AND tracked_at::date BETWEEN $1 AND $2 GROUP BY campaign) b ON b.campaign=c.campaign
      LEFT JOIN (SELECT campaign, COUNT(*) as spam_complaints FROM email_tracking WHERE event_type='spam_complaint' AND tracked_at::date BETWEEN $1 AND $2 GROUP BY campaign) sp ON sp.campaign=c.campaign
      ...
      LEFT JOIN (SELECT campaign, COUNT(*) as clicks, COUNT(DISTINCT recipient) as unique_clickers FROM email_tracking WHERE event_type='click' AND tracked_at::date BETWEEN $1 AND $2 GROUP BY campaign) cl ON cl.campaign=c.campaign
    ...
    const byEmail = await pool.query(`
      SELECT c.campaign, c.email_id, c.sends,
        COALESCE(d.delivered,0) as delivered, COALESCE(b.bounced,0) as bounced, COALESCE(sp.spam_complaints,0) as spam_complaints,
        COALESCE(o.opens,0) as opens, COALESCE(o.unique_openers,0) as unique_openers,
        COALESCE(cl.clicks,0) as clicks, COALESCE(cl.unique_clickers,0) as unique_clickers,
        CASE WHEN c.sends>0 THEN ROUND(COALESCE(d.delivered,0)::numeric/c.sends*100,1) ELSE 0 END as delivery_rate,
      ...
      LEFT JOIN (SELECT campaign, email_id, COUNT(*) as delivered FROM email_tracking WHERE event_type='delivered' AND tracked_at::date BETWEEN $1 AND $2 GROUP BY campaign, email_id) d ON d.campaign=c.campaign AND d.email_id=c.email_id
      LEFT JOIN (SELECT campaign, email_id, COUNT(*) as bounced FROM email_tracking WHERE event_type IN ('bounced','soft_bounce','failed','rejected','suppressed') AND tracked_at::date BETWEEN $1 AND $2 GROUP BY campaign, email_id) b ON b.campaign=c.campaign AND b.email_id=c.email_id
      LEFT JOIN (SELECT campaign, email_id, COUNT(*) as spam_complaints FROM email_tracking WHERE event_type='spam_complaint' AND tracked_at::date BETWEEN $1 AND $2 GROUP BY campaign, email_id) sp ON sp.campaign=c.campaign AND sp.email_id=c.email_id
      ...
      LEFT JOIN (SELECT campaign, email_id, COUNT(*) as clicks, COUNT(DISTINCT recipient) as unique_clickers FROM email_tracking WHERE event_type='click' AND tracked_at::date BETWEEN $1 AND $2 GROUP BY campaign, email_id) cl ON cl.campaign=c.campaign AND cl.email_id=c.email_id
...
app.post("/api/webhooks/outbound/register", auth, async (req, res) => {
  try {
    ...
    if(url.includes('localhost')||url.includes('127.0.0.1')||url.includes('0.0.0.0')) return res.status(400).json({error:'Internal URLs not allowed'});
...
// ====================== ADS HUB (unified ad management) ======================
app.get("/api/ads/unified", auth, async (req, res) => {
  try {
    const { start, end } = dr(req.query);
    const platforms = await pool.query(`SELECT platform, SUM(spend) as spend, SUM(impressions) as impressions, SUM(clicks) as clicks, SUM(conversions) as conversions, SUM(conversion_value) as revenue,
      ...
      FROM ad_spend_daily WHERE date BETWEEN $1 AND $2 GROUP BY platform ORDER BY spend DESC`,[start,end]);
    const campaigns = await pool.query(`SELECT platform, campaign_name, campaign_id, SUM(spend) as spend, SUM(impressions) as impressions, SUM(clicks) as clicks, SUM(conversions) as conversions, SUM(conversion_value) as revenue,
      CASE WHEN SUM(impressions)>0 THEN ROUND(SUM(clicks)::numeric/SUM(impressions)*100,2) ELSE 0 END as ctr,
      CASE WHEN SUM(clicks)>0 THEN ROUND(SUM(spend)/SUM(clicks),2) ELSE 0 END as cpc,
      CASE WHEN SUM(spend)>0 THEN ROUND(SUM(conversion_value)/SUM(spend),2) ELSE 0 END as roas
      FROM ad_spend_daily WHERE date BETWEEN $1 AND $2 GROUP BY platform, campaign_name, campaign_id ORDER BY spend DESC LIMIT 50`,[start,end]);
    const dailyTotals = await pool.query(`SELECT date, SUM(spend) as spend, SUM(clicks) as clicks, SUM(conversions) as conversions, SUM(conversion_value) as revenue FROM ad_spend_daily WHERE date BETWEEN $1 AND $2 GROUP BY date ORDER BY date`,[start,end]);
    const t = platforms.rows.reduce((a,p) => ({spend:a.spend+(+p.spend||0),impressions:a.impressions+(+p.impressions||0),clicks:a.clicks+(+p.clicks||0),conversions:a.conversions+(+p.conversions||0),revenue:a.revenue+(+p.revenue||0)}),{spend:0,impressions:0,clicks:0,conversions:0,revenue:0});
    t.ctr = t.impressions>0 ? Math.round(t.clicks/t.impressions*10000)/100 : 0;
    t.cpc = t.clicks>0 ? Math.round(t.spend/t.clicks*100)/100 : 0;
    t.cpa = t.conversions>0 ? Math.round(t.spend/t.conversions*100)/100 : 0;
    t.roas = t.spend>0 ? Math.round(t.revenue/t.spend*100)/100 : 0;
    res.json({ totals: t, platforms: platforms.rows, campaigns: campaigns.rows, dailyTotals: dailyTotals.rows });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.get("/api/ads/ai-copilot", auth, async (req, res) => {
  try {
    const { start, end } = dr(req.query);
    const platforms = await pool.query(`SELECT platform, SUM(spend) as spend, SUM(clicks) as clicks, SUM(conversions) as conversions, SUM(conversion_value) as revenue, SUM(impressions) as impressions FROM ad_spend_daily WHERE date BETWEEN $1 AND $2 GROUP BY platform`,[start,end]);
    const campaigns = await pool.query(`SELECT platform, campaign_name, SUM(spend) as spend, SUM(clicks) as clicks, SUM(conversions) as conversions, SUM(conversion_value) as revenue, SUM(impressions) as impressions FROM ad_spend_daily WHERE date BETWEEN $1 AND $2 GROUP BY platform, campaign_name ORDER BY spend DESC LIMIT 20`,[start,end]);
    const weeklyTrend = await pool.query(`SELECT campaign_name, DATE_TRUNC('week',date)::date as week, SUM(spend) as spend, CASE WHEN SUM(impressions)>0 THEN SUM(clicks)::numeric/SUM(impressions)*100 ELSE 0 END as ctr, CASE WHEN SUM(spend)>0 THEN SUM(conversion_value)/SUM(spend) ELSE 0 END as roas FROM ad_spend_daily WHERE date BETWEEN $1 AND $2 GROUP BY campaign_name, week ORDER BY campaign_name, week`,[start,end]);
    const actions = [], insights = [];
    const totalSpend = platforms.rows.reduce((s,p)=>s+(+p.spend||0),0);
    const bestP = [...platforms.rows].sort((a,b)=>(+b.revenue/Math.max(+b.spend,1))-(+a.revenue/Math.max(+a.spend,1)))[0];
    const worstP = [...platforms.rows].sort((a,b)=>(+a.revenue/Math.max(+a.spend,1))-(+b.revenue/Math.max(+b.spend,1)))[0];
    if(bestP&&worstP&&bestP.platform!==worstP.platform){
      const br=+bestP.revenue/Math.max(+bestP.spend,1), wr=+worstP.revenue/Math.max(+worstP.spend,1);
      if(br>wr*1.5){const shift=Math.round(+worstP.spend*0.3); actions.push({type:'budget_shift',priority:'high',title:'Shift Budget to '+bestP.platform,detail:bestP.platform+' ROAS ('+br.toFixed(1)+'x) outperforms '+worstP.platform+' ('+wr.toFixed(1)+'x). Move $'+shift+' for est. +$'+Math.round(shift*(br-wr))+' revenue.',platform:bestP.platform});}
    }
    for(const camp of campaigns.rows){
      const roas=+camp.spend>0?+camp.revenue/+camp.spend:0, ctr=+camp.impressions>0?+camp.clicks/+camp.impressions*100:0;
      if(roas<1&&+camp.spend>50) actions.push({type:'pause',priority:'high',title:'Pause: '+(camp.campaign_name||'Unnamed'),detail:'ROAS '+roas.toFixed(1)+'x on $'+Math.round(+camp.spend)+'. Losing money.',platform:camp.platform});
      else if(roas>=3) actions.push({type:'scale',priority:'low',title:'Scale: '+(camp.campaign_name||'Unnamed'),detail:'ROAS '+roas.toFixed(1)+'x — increase budget 20-30%.',platform:camp.platform});
      if(ctr<0.5&&+camp.impressions>1000) actions.push({type:'creative',priority:'medium',title:'Refresh Creative: '+(camp.campaign_name||'Unnamed'),detail:'CTR '+ctr.toFixed(2)+'% is very low. Test new creative.',platform:camp.platform});
    }
    const campWeeks={};
    for(const r of weeklyTrend.rows){if(!campWeeks[r.campaign_name])campWeeks[r.campaign_name]=[];campWeeks[r.campaign_name].push(r);}
    for(const[name,weeks]of Object.entries(campWeeks)){
      if(weeks.length>=3){const recent=weeks.slice(-2),earlier=weeks.slice(0,-2);
        const rc=recent.reduce((s,w)=>s+(+w.ctr),0)/recent.length, ec=earlier.reduce((s,w)=>s+(+w.ctr),0)/earlier.length;
        if(ec>0&&rc<ec*0.6) actions.push({type:'fatigue',priority:'high',title:'Ad Fatigue: '+name,detail:'CTR dropped '+Math.round((1-rc/ec)*100)+'% over recent weeks. Replace creative.',platform:weeks[0]?.platform||''});
      }
    }
    if(totalSpend>0){const or=platforms.rows.reduce((s,p)=>s+(+p.revenue||0),0)/totalSpend; insights.push({type:or>=3?'positive':or>=1.5?'neutral':'negative',text:'Overall ROAS: '+or.toFixed(1)+'x. '+(or>=3?'Excellent.':or>=1.5?'Decent — optimize underperformers.':'Below target — review creatives.')});}
    actions.sort((a,b)=>(a.priority==='high'?0:a.priority==='medium'?1:2)-(b.priority==='high'?0:b.priority==='medium'?1:2));
    res.json({ insights, actions, totalSpend: Math.round(totalSpend), campaignsAnalyzed: campaigns.rows.length });
  } catch(e) { res.status(500).json({ error: e.message }); }
});


// ====================== DAYPARTING ANALYTICS ======================

// Hourly ad data table migration
// Note: Meta API supports hourly_stats_aggregated_by_advertiser_time_zone
// Google Ads supports segments.hour for hourly breakdown

// Sync hourly data from Meta Ads
app.post("/api/ads/sync-hourly", auth, syncRateLimit, async (req, res) => {
  try {
    const mc = await getCreds('meta_ads');
    const token = mc.access_token || process.env.META_ACCESS_TOKEN;
    const account = mc.ad_account_id || process.env.META_AD_ACCOUNT_ID;
    if (!token || !account) return res.status(400).json({ error: "Meta Ads not configured" });
    
    const since = req.body.date_from || new Date(Date.now()-14*864e5).toISOString().split('T')[0];
    const until = req.body.date_to || new Date().toISOString().split('T')[0];
    
    // Pull hourly breakdown from Meta
    const resp = await axios.get('https://graph.facebook.com/v18.0/'+account+'/insights', {
      params: {
    ...
    }).catch(e => ({ data: { data: [] }, error: e.response?.data?.error?.message || e.message }));
    ...
    for (const row of (resp.data?.data || [])) {
      ...
      if (!hour) continue;
      // Parse hour range "00:00:00 - 00:59:59" → extract hour number
      const hourNum = parseInt(hour.split(':')[0]) || 0;
      const conversions = (row.actions||[]).find(a=>a.action_type==='purchase')?.value || 0;
      const convValue = (row.action_values||[]).find(a=>a.action_type==='purchase')?.value || 0;
      
      await pool.query(`INSERT INTO ad_spend_hourly(date,hour,platform,campaign_name,spend,impressions,clicks,conversions,conversion_value)
        VALUES($1,$2,'meta',$3,$4,$5,$6,$7,$8)
        ON CONFLICT(date,hour,platform,campaign_name) DO UPDATE SET spend=$4,impressions=$5,clicks=$6,conversions=$7,conversion_value=$8`,
        [row.date_start, hourNum, row.campaign_name||'All Campaigns', +(row.spend||0), +(row.impressions||0), +(row.clicks||0), +conversions, +convValue]);
      synced++;
    }
    
    res.json({ synced, source: 'meta', period: { since, until } });
  } catch(e) { res.status(500).json({ error: e.response?.data?.error?.message || e.message }); }
});

// Get dayparting analytics
app.get("/api/ads/dayparting", auth, async (req, res) => {
  try {
    const { start, end } = dr(req.query);
    
    // ── By Day of Week (from daily data — always available) ──
    const byDow = await pool.query(`SELECT EXTRACT(DOW FROM date) as dow,
      ...
      CASE WHEN SUM(spend)>0 THEN ROUND(SUM(conversion_value)/SUM(spend),2) ELSE 0 END as roas,
      CASE WHEN SUM(conversions)>0 THEN ROUND(SUM(spend)/SUM(conversions),2) ELSE 0 END as cpa
      FROM ad_spend_daily WHERE date BETWEEN $1 AND $2
      GROUP BY dow, day_name ORDER BY dow`,[start,end]);
    
    // ── By Hour (from hourly data if available) ──
    const byHour = await pool.query(`SELECT hour,
      COALESCE(SUM(spend),0) as spend, COALESCE(SUM(impressions),0) as impressions,
      COALESCE(SUM(clicks),0) as clicks, COALESCE(SUM(conversions),0) as conversions,
      COALESCE(SUM(conversion_value),0) as revenue,
      CASE WHEN SUM(impressions)>0 THEN ROUND(SUM(clicks)::numeric/SUM(impressions)*100,2) ELSE 0 END as ctr,
      CASE WHEN SUM(spend)>0 THEN ROUND(SUM(conversion_value)/SUM(spend),2) ELSE 0 END as roas,
      CASE WHEN SUM(conversions)>0 THEN ROUND(SUM(spend)/SUM(conversions),2) ELSE 0 END as cpa
      FROM ad_spend_hourly WHERE date BETWEEN $1 AND $2
      GROUP BY hour ORDER BY hour`,[start,end]);
    
    // ── Heatmap: Day × Hour (best combination) ──
    const heatmap = await pool.query(`SELECT EXTRACT(DOW FROM date) as dow,
      TO_CHAR(date, 'Dy') as day_name, hour,
      COALESCE(SUM(spend),0) as spend, COALESCE(SUM(clicks),0) as clicks,
      COALESCE(SUM(conversions),0) as conversions, COALESCE(SUM(conversion_value),0) as revenue,
      CASE WHEN SUM(spend)>0 THEN ROUND(SUM(conversion_value)/SUM(spend),2) ELSE 0 END as roas
      FROM ad_spend_hourly WHERE date BETWEEN $1 AND $2
      GROUP BY dow, day_name, hour ORDER BY dow, hour`,[start,end]);
    
    // ── Best campaigns by day of week ──
    const campByDow = await pool.query(`SELECT EXTRACT(DOW FROM date) as dow,
      TO_CHAR(date, 'Dy') as day_name, campaign_name, platform,
      SUM(spend) as spend, SUM(clicks) as clicks, SUM(conversions) as conversions,
      SUM(conversion_value) as revenue,
      CASE WHEN SUM(spend)>0 THEN ROUND(SUM(conversion_value)/SUM(spend),2) ELSE 0 END as roas
      FROM ad_spend_daily WHERE date BETWEEN $1 AND $2 AND spend>0
      GROUP BY dow, day_name, campaign_name, platform
      HAVING SUM(spend)>10
      ORDER BY dow, roas DESC`,[start,end]);
    
    // ── Best campaigns by hour ──
    const campByHour = await pool.query(`SELECT hour, campaign_name, platform,
      SUM(spend) as spend, SUM(clicks) as clicks, SUM(conversions) as conversions,
      SUM(conversion_value) as revenue,
      CASE WHEN SUM(spend)>0 THEN ROUND(SUM(conversion_value)/SUM(spend),2) ELSE 0 END as roas
      FROM ad_spend_hourly WHERE date BETWEEN $1 AND $2 AND spend>0
      GROUP BY hour, campaign_name, platform
      HAVING SUM(spend)>5
      ORDER BY hour, roas DESC`,[start,end]);
    ...
    if (byDow.rows.length >= 5) {
      const sorted = [...byDow.rows].sort((a,b) => +b.roas - +a.roas);
      const best = sorted[0];
      const worst = sorted[sorted.length-1];
      
      if (+best.roas > 0 && +worst.roas >= 0) {
        recommendations.push({
          type: 'increase_bid', priority: 'high',
          title: 'Increase bids on ' + dayNames[+best.dow],
          detail: dayNames[+best.dow] + ' has your best ROAS (' + (+best.roas).toFixed(1) + 'x) with ' + Math.round(+best.conversions) + ' conversions. Increase bids 15-25% on this day to capture more volume.',
          metric: 'roas', value: +best.roas, day: dayNames[+best.dow]
        });
      }
      if (+worst.roas < 1 && +worst.spend > 20) {
        recommendations.push({
          type: 'decrease_bid', priority: 'high',
          title: 'Reduce bids on ' + dayNames[+worst.dow],
          detail: dayNames[+worst.dow] + ' has your worst ROAS (' + (+worst.roas).toFixed(1) + 'x) spending $' + Math.round(+worst.spend) + '. Decrease bids 20-30% or pause campaigns on this day.',
          metric: 'roas', value: +worst.roas, day: dayNames[+worst.dow]
        });
      }
      
      // Weekend vs weekday
      const weekday = byDow.rows.filter(r => +r.dow >= 1 && +r.dow <= 5);
      const weekend = byDow.rows.filter(r => +r.dow === 0 || +r.dow === 6);
      const wdRoas = weekday.reduce((s,r)=>s+(+r.revenue||0),0) / Math.max(weekday.reduce((s,r)=>s+(+r.spend||0),0), 1);
      const weRoas = weekend.reduce((s,r)=>s+(+r.revenue||0),0) / Math.max(weekend.reduce((s,r)=>s+(+r.spend||0),0), 1);
      if (Math.abs(wdRoas - weRoas) > 0.5) {
        const better = wdRoas > weRoas ? 'weekdays' : 'weekends';
        const betterRoas = Math.max(wdRoas, weRoas);
        const worseRoas = Math.min(wdRoas, weRoas);
        recommendations.push({
          type: 'schedule', priority: 'medium',
          title: 'Shift budget to ' + better,
          detail: better.charAt(0).toUpperCase() + better.slice(1) + ' ROAS (' + betterRoas.toFixed(1) + 'x) significantly outperforms ' + (better==='weekdays'?'weekends':'weekdays') + ' (' + worseRoas.toFixed(1) + 'x). Consider 60/40 budget split favoring ' + better + '.',
          metric: 'roas_diff', value: betterRoas - worseRoas
        });
      }
    }
    
    // Hourly recommendations
    if (byHour.rows.length >= 12) {
      const sortedH = [...byHour.rows].sort((a,b) => +b.roas - +a.roas);
      const bestH = sortedH.filter(h => +h.conversions > 0).slice(0, 3);
      const worstH = sortedH.filter(h => +h.spend > 5 && +h.roas < 1).slice(-3);
      
      if (bestH.length > 0) {
        const hourLabels = bestH.map(h => {
          const hr = +h.hour;
          return hr === 0 ? '12 AM' : hr < 12 ? hr + ' AM' : hr === 12 ? '12 PM' : (hr-12) + ' PM';
        }).join(', ');
        recommendations.push({
          type: 'peak_hours', priority: 'high',
          title: 'Peak conversion hours: ' + hourLabels,
          detail: 'Your best converting hours are ' + hourLabels + ' with an average ROAS of ' + (bestH.reduce((s,h)=>s+(+h.roas),0)/bestH.length).toFixed(1) + 'x. Set bid adjustments +20% during these hours.',
          metric: 'peak', value: bestH[0]?.roas
        });
      }
      
      if (worstH.length > 0) {
        const worstLabels = worstH.map(h => {
          const hr = +h.hour;
          return hr === 0 ? '12 AM' : hr < 12 ? hr + ' AM' : hr === 12 ? '12 PM' : (hr-12) + ' PM';
        }).join(', ');
        const wastedSpend = worstH.reduce((s,h)=>s+(+h.spend||0),0);
        recommendations.push({
          type: 'dead_hours', priority: 'medium',
          title: 'Low-performing hours: ' + worstLabels,
          detail: 'These hours have ROAS below 1x with $' + Math.round(wastedSpend) + ' wasted. Reduce bids 30-50% or pause ads during ' + worstLabels + '.',
          metric: 'waste', value: wastedSpend
        });
      }
    }
    
    // Best campaigns per time slot
    const topCampsByDay = {};
    for (const row of campByDow.rows) {
      const key = +row.dow;
      if (!topCampsByDay[key]) topCampsByDay[key] = [];
      if (topCampsByDay[key].length < 3) topCampsByDay[key].push(row);
    }
    
    const topCampsByHour = {};
    for (const row of campByHour.rows) {
      const key = +row.hour;
      if (!topCampsByHour[key]) topCampsByHour[key] = [];
      if (topCampsByHour[key].length < 3) topCampsByHour[key].push(row);
    }
    
    recommendations.sort((a,b) => (a.priority==='high'?0:a.priority==='medium'?1:2) - (b.priority==='high'?0:b.priority==='medium'?1:2));
    
    res.json({
      byDayOfWeek: byDow.rows,
      byHour: byHour.rows,
      heatmap: heatmap.rows,
      topCampsByDay,
      topCampsByHour,
      recommendations,
      hasHourlyData: byHour.rows.length > 0,
    });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
// Generate AI analysis from business data using Claude
async function claudeAnalyze(systemPrompt, userPrompt) {
...
// AI Weekly Report — comprehensive business analysis
app.post("/api/ai/weekly-report", auth, async (req, res) => {
  try {
    const thisWeek = (await pool.query("SELECT SUM(revenue) as rev, SUM(net_profit) as profit, SUM(orders_count) as orders, SUM(new_customers) as new_cust, SUM(ad_spend) as ads, SUM(cogs) as cogs, SUM(refund_total) as refunds, SUM(email_revenue) as email_rev, SUM(tax_total) as tax FROM daily_metrics WHERE date>=CURRENT_DATE-INTERVAL '7 days'")).rows[0];
    const lastWeek = (await pool.query("SELECT SUM(revenue) as rev, SUM(net_profit) as profit, SUM(orders_count) as orders, SUM(new_customers) as new_cust, SUM(ad_spend) as ads FROM daily_metrics WHERE date>=CURRENT_DATE-INTERVAL '14 days' AND date<CURRENT_DATE-INTERVAL '7 days'")).rows[0];
    const topProducts = (await pool.query("SELECT p.name, SUM(oi.subtotal) as rev, COUNT(oi.id) as units FROM order_items oi JOIN products p ON p.id=oi.product_id JOIN orders o ON o.id=oi.order_id WHERE o.order_date>=NOW()-INTERVAL '7 days' GROUP BY p.name ORDER BY rev DESC LIMIT 5")).rows;
    const lowStock = (await pool.query("SELECT name, stock_quantity FROM products WHERE stock_quantity>0 AND stock_quantity<COALESCE(reorder_point,10) AND is_active=true")).rows;
    const emailStats = (await pool.query("SELECT COUNT(*) FILTER(WHERE event_type='send') as sends, COUNT(DISTINCT recipient) FILTER(WHERE event_type='open') as opens FROM email_tracking WHERE tracked_at>=NOW()-INTERVAL '7 days'")).rows[0];
    const adPlatforms = (await pool.query("SELECT platform, SUM(spend) as spend, SUM(conversion_value) as rev FROM ad_spend_daily WHERE date>=CURRENT_DATE-INTERVAL '7 days' GROUP BY platform ORDER BY spend DESC")).rows;

    const dataStr = JSON.stringify({ thisWeek, lastWeek, topProducts, lowStock, emailStats, adPlatforms }, null, 2);

    const report = await claudeAnalyze(
      "You are the AI Finance Director for The Vitamin Shots, a DTC vegan supplement brand selling Vitamin Shots, Glam Dust, and Vitamin Sprinkles. Generate a concise, actionable weekly business report. Use specific numbers. Format with markdown headers. Include: Executive Summary, Revenue & Profit Analysis (week-over-week changes), Marketing Performance (ad ROAS by platform), Email Performance, Top Products, Inventory Alerts, and Top 3 Action Items for next week. Be direct and specific — the founder reads this on mobile.",
      "Here is this week's data. Generate the weekly report:\n" + dataStr
    );

    res.json({ report, generatedAt: new Date().toISOString(), data: { thisWeek, lastWeek } });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// AI On-Demand Analysis — ask anything about the business
app.post("/api/ai/analyze", auth, async (req, res) => {
  try {
    ...
    // Gather context data
    const overview = (await pool.query("SELECT SUM(revenue) as rev, SUM(net_profit) as profit, SUM(orders_count) as orders, SUM(ad_spend) as ads, SUM(new_customers) as new_cust FROM daily_metrics WHERE date>=CURRENT_DATE-INTERVAL '30 days'")).rows[0];
    let products = []; try { products = (await pool.query("SELECT name, price, stock_quantity FROM products WHERE is_active=true")).rows; } catch { try { products = (await pool.query("SELECT name, price, stock_quantity FROM products")).rows; } catch { } }
    const topCampaigns = (await pool.query("SELECT platform, campaign_name, SUM(spend) as spend, SUM(conversion_value) as rev FROM ad_spend_daily WHERE date>=CURRENT_DATE-INTERVAL '30 days' GROUP BY platform, campaign_name ORDER BY spend DESC LIMIT 10")).rows;
    const customers = (await pool.query("SELECT COUNT(*) as total, AVG(total_revenue) as avg_rev, AVG(total_orders) as avg_orders FROM customers WHERE total_orders>0")).rows[0];

    const context = JSON.stringify({ last30Days: overview, products, topAdCampaigns: topCampaigns, customerStats: customers }, null, 2);

    const answer = await claudeAnalyze(
      "You are the AI analyst for The Vitamin Shots, a DTC vegan supplement brand. Answer business questions using the provided data. Be specific with numbers. Give actionable advice. Keep answers concise (under 300 words). Format with bullet points where helpful.",
      "Business data (last 30 days):\n" + context + "\n\nQuestion: " + question
    );

    res.json({ answer, question, generatedAt: new Date().toISOString() });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// AI Dashboard Summary — quick daily brief
app.get("/api/ai/daily-brief", auth, async (req, res) => {
  try {
    const today = (await pool.query("SELECT revenue, net_profit, orders_count, ad_spend, new_customers FROM daily_metrics WHERE date=CURRENT_DATE")).rows[0];
    const yesterday = (await pool.query("SELECT revenue, net_profit, orders_count, ad_spend FROM daily_metrics WHERE date=CURRENT_DATE-1")).rows[0];
    const avg = (await pool.query("SELECT AVG(revenue) as rev, AVG(orders_count) as orders FROM daily_metrics WHERE date>=CURRENT_DATE-INTERVAL '30 days'")).rows[0];

    if (!today && !yesterday) return res.json({ brief: "No data for today yet. Sync WooCommerce to get today's numbers.", generatedAt: new Date().toISOString() });

    const d = today || yesterday;
    const brief = await claudeAnalyze(
      "You are a business dashboard AI. Give a 2-3 sentence daily brief. Be specific with $amounts. Compare to yesterday and 30-day average. Mention anything unusual. Keep it casual and mobile-friendly.",
      JSON.stringify({ today: d, yesterday, avg30d: avg })
    );

    res.json({ brief, generatedAt: new Date().toISOString() });
  } catch(e) { res.json({ brief: "Could not generate brief: " + e.message, generatedAt: new Date().toISOString() }); }
});


// ====================== COMMAND HUB — ALL SOFTWARE PRODUCTS ======================

// Product registry
const HUB_PRODUCTS = [
  { id:'botdolphin', name:'BotDolphin', icon:'🐬', desc:'AI chatbot platform', configKeys:['url','api_key'], defaultUrl:'https://app.botdolphin.com' },
  { id:'webcomplifier', name:'WebComplifier', icon:'♿', desc:'WCAG accessibility scanner', configKeys:['url','api_key'], defaultUrl:'' },
  { id:'sitescannify', name:'Sitescannify', icon:'🔍', desc:'Conversion & SEO audit', configKeys:['url','api_key'], defaultUrl:'' },
  { id:'audience_iq', name:'Audience IQ', icon:'🧠', desc:'CLV modeling + Meta audiences', configKeys:['url','api_key'], defaultUrl:'' },
  { id:'zerorank', name:'ZeroRank', icon:'🎯', desc:'AI search visibility tracker', configKeys:['url','api_key'], defaultUrl:'' },
  { id:'gsc_reporter', name:'GSC SEO Reporter', icon:'📈', desc:'Search Console reporting', configKeys:['url','api_key'], defaultUrl:'' },
  { id:'amazon_tool', name:'Amazon Seller Tool', icon:'📦', desc:'Product research & analytics', configKeys:['url','api_key'], defaultUrl:'' },
  { id:'meta_ads', name:'Meta Ads Tool', icon:'📘', desc:'Facebook/Instagram ads analytics', configKeys:['url','api_key'], defaultUrl:'' },
  { id:'tvs_reviews', name:'TVS Reviews', icon:'⭐', desc:'Customer reviews plugin', configKeys:['url','api_key'], defaultUrl:'https://thevitaminshots.com/wp-json/tvs-hub/v1' },
  { id:'popper', name:'Popper', icon:'🪧', desc:'Popups & on-site conversion', configKeys:['url','api_key'], defaultUrl:'https://thevitaminshots.com/wp-json/tvs-hub/v1' },
  { id:'nexus_tax', name:'Sales Tax', icon:'🧾', desc:'Nexus tracking & tax collected', configKeys:['url','api_key'], defaultUrl:'https://thevitaminshots.com/wp-json/tvs-hub/v1' },
  { id:'dispute_shield', name:'Dispute Shield', icon:'🛡️', desc:'Chargeback management', configKeys:['url','api_key'], defaultUrl:'' },
  { id:'wellness_app', name:'TVS Wellness App', icon:'💚', desc:'iOS/Android wellness app', configKeys:['url','api_key'], defaultUrl:'' },
  { id:'admin_panel', name:'Admin Panel', icon:'🛠️', desc:'App admin dashboard', configKeys:['url','api_key'], defaultUrl:'' },
  { id:'tvs_store', name:'TVS Store', icon:'🛒', desc:'WooCommerce store', configKeys:['url','api_key'], defaultUrl:'https://thevitaminshots.com' },
];

// Fetch stats from a connected product
async function fetchProductStats(product) {
  try {
    // TVS Store IS this backend's own WooCommerce store — serve real stats from our DB
    // instead of trying to reach a /api/hub/stats endpoint that the storefront doesn't expose.
    if (product.id === 'tvs_store') {
      const m = (await pool.query("SELECT COALESCE(SUM(revenue),0) as revenue, COALESCE(SUM(net_profit),0) as net_profit, COALESCE(SUM(orders_count),0) as orders, COALESCE(SUM(new_customers),0) as new_customers, COALESCE(SUM(ad_spend),0) as ad_spend FROM daily_metrics WHERE date>=CURRENT_DATE-INTERVAL '30 days'")).rows[0];
      return {
        connected: true,
        metrics: {
          revenue_30d: Math.round(+m.revenue),
          net_profit_30d: Math.round(+m.net_profit),
          orders_30d: +m.orders,
          new_customers_30d: +m.new_customers,
          ad_spend_30d: Math.round(+m.ad_spend)
        },
        lastFetched: new Date().toISOString()
      };
    }
    const creds = await getCreds('hub_' + product.id);
    const url = creds.url || product.defaultUrl;
    const apiKey = creds.api_key || process.env['HUB_' + product.id.toUpperCase() + '_KEY'];
    if (!url) return { connected: false, error: 'URL not configured' };
    const resp = await axios.get(url.replace(/\/$/, '') + '/api/hub/stats', {
      headers: { 'x-hub-key': apiKey, 'Authorization': 'Bearer ' + apiKey },
      timeout: 8000
    }).catch(e => ({ data: null, error: e.message }));
    if (!resp.data) return { connected: false, error: resp.error || 'No response' };
    return { connected: true, ...resp.data, lastFetched: new Date().toISOString() };
  } catch(e) { return { connected: false, error: e.message }; }
}

// Get all product statuses
app.get("/api/hub/products", auth, async (req, res) => {
  try {
    const results = [];
    for (const p of HUB_PRODUCTS) {
      const stats = await fetchProductStats(p);
...
// Get single product stats (for tab view)
app.get("/api/hub/product/:id", auth, async (req, res) => {
  try {
    const product = HUB_PRODUCTS.find(p => p.id === req.params.id);
    if (!product) return res.status(404).json({ error: 'Product not found' });
    const stats = await fetchProductStats(product);
    res.json({ product: { ...product, stats } });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Quick action — proxy command to a product
app.post("/api/hub/action/:id", auth, async (req, res) => {
  try {
    const product = HUB_PRODUCTS.find(p => p.id === req.params.id);
    if (!product) return res.status(404).json({ error: 'Product not found' });
    const creds = await getCreds('hub_' + product.id);
    ...
    if (!url || !apiKey) return res.status(400).json({ error: 'Product not configured' });
    const resp = await axios.post(url.replace(/\/$/, '') + '/api/hub/action', req.body, {
      headers: { 'x-hub-key': apiKey }, timeout: 15000
    }).catch(e => ({ data: { error: e.message } }));
    res.json(resp.data);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Hub health check — all products at once
app.get("/api/hub/health", auth, async (req, res) => {
  try {
    const checks = await Promise.allSettled(HUB_PRODUCTS.map(async p => {
      const creds = await getCreds('hub_' + p.id);
      const url = creds.url || p.defaultUrl;
      if (!url) return { id: p.id, status: 'not_configured' };
      try {
        const hdrs = { 'x-hub-key': creds.api_key, 'Authorization': 'Bearer ' + creds.api_key };
        let r;
        try { r = await axios.get(url.replace(/\/+$/, '') + '/api/hub/health', { timeout: 5000, headers: hdrs }); }
        catch { r = await axios.get(url.replace(/\/+$/, '') + '/hub/health', { timeout: 5000, headers: hdrs }); }
        return { id: p.id, status: 'healthy', responseTime: r.headers?.['x-response-time'] };
      } catch(e) { return { id: p.id, status: e.response?.status === 401 ? 'auth_error' : 'down', error: e.message }; }
    }));
    res.json({ products: checks.map(c => c.value || { status: 'error' }) });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Weekly digest covering ALL products — sent via email
app.post("/api/hub/weekly-digest", auth, async (req, res) => {
  try {
    // Gather data from all connected products
    const productData = {};
    for (const p of HUB_PRODUCTS) {
      productData[p.id] = await fetchProductStats(p);
    }
    // TVS Store data (from our own DB)
    const tw = (await pool.query("SELECT SUM(revenue) as rev, SUM(net_profit) as profit, SUM(orders_count) as orders, SUM(new_customers) as new_cust, SUM(ad_spend) as ads FROM daily_metrics WHERE date>=CURRENT_DATE-INTERVAL '7 days'")).rows[0];
    productData.tvs_finance = tw;
    
    // Generate AI digest
    let digest = '';
    if (process.env.ANTHROPIC_API_KEY) {
      digest = await claudeAnalyze(
        "You are the CEO briefing AI for Devin, who runs The Vitamin Shots (DTC supplements) and multiple SaaS products. Write a concise weekly email digest covering all products. Use specific numbers. Format with headers per product. End with top 3 priorities for next week.",
        "All product data this week:\n" + JSON.stringify(productData, null, 2)
      );
    } else {
      digest = "Weekly Digest\n\nTVS Store: $" + Math.round(+(tw?.rev||0)) + " revenue, " + Math.round(+(tw?.orders||0)) + " orders, $" + Math.round(+(tw?.profit||0)) + " profit.\n\nConnect ANTHROPIC_API_KEY for AI-generated insights.";
    }
    
    // Send email — track real delivery results
    let users; try { users = await pool.query("SELECT email FROM users WHERE is_active=true"); } catch { users = await pool.query("SELECT email FROM users"); }
    let sentCount = 0; let lastError = null;
    for (const u of users.rows) {
      const r = await sendEmail({ to: u.email, subject: '📊 Weekly Business Digest — All Products', html: '<div style="font-family:Arial;max-width:600px;margin:0 auto;"><h1 style="color:#f1c349;">Weekly Business Digest</h1><pre style="white-space:pre-wrap;font-size:14px;line-height:1.6;">' + digest.replace(/\n/g, '<br>') + '</pre><hr><p style="color:#888;font-size:12px;">Vitamin Shots Finance Minister · Command Hub</p></div>' });
      if (r?.success) sentCount++; else lastError = r?.error || 'unknown error';
    }
    // If nothing was delivered, surface the real reason instead of a false success
    if (sentCount === 0) return res.status(500).json({ error: 'Digest generated but no email was delivered. ' + (lastError || 'Add email credentials in Settings → Integrations.'), digest });
    res.json({ sent: true, recipients: sentCount, digest });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ====================== CEO COCKPIT — cross-product audit, AI reports, chat ======================

// Fetch a product's audit (deep self-audit). TVS Store is computed locally from our own data.
async function fetchProductAudit(product) {
  try {
    if (product.id === 'tvs_store') {
      const m = (await pool.query("SELECT COALESCE(SUM(revenue),0) rev, COALESCE(SUM(net_profit),0) profit, COALESCE(SUM(gross_profit),0) gp, COALESCE(SUM(refund_total),0) refunds, COALESCE(SUM(ad_spend),0) ads, COALESCE(SUM(orders_count),0) orders FROM daily_metrics WHERE date>=CURRENT_DATE-INTERVAL '30 days'")).rows[0];
      let missingCogs=0, oos=0, low=0;
      try { missingCogs = +(await pool.query("SELECT COUNT(*) c FROM products WHERE is_active=true AND COALESCE(landed_cost,0)=0 AND COALESCE(cogs,0)=0")).rows[0].c; } catch {}
      try { oos = +(await pool.query("SELECT COUNT(*) c FROM products WHERE is_active=true AND stock_quantity<=0")).rows[0].c; } catch {}
      try { low = +(await pool.query("SELECT COUNT(*) c FROM products WHERE is_active=true AND stock_quantity>0 AND stock_quantity<reorder_point")).rows[0].c; } catch {}
      const rev=+m.rev, ads=+m.ads, gp=+m.gp, refunds=+m.refunds;
      const mer = ads>0 ? rev/ads : 0, refundRate = rev>0 ? refunds/rev*100 : 0, marginPct = rev>0 ? gp/rev*100 : 0;
      const findings=[], recs=[]; let score=100;
      if (missingCogs>0){ score-=15; findings.push({severity:'high',title:missingCogs+' products missing COGS',detail:'Profit on these is overstated until cost is set.'}); recs.push('Add landed cost for '+missingCogs+' product(s) so profit is accurate.'); }
      if (oos>0){ score-=15; findings.push({severity:'high',title:oos+' product(s) out of stock',detail:'Losing sales while unavailable.'}); recs.push('Restock '+oos+' out-of-stock product(s).'); }
      if (low>0){ score-=8; findings.push({severity:'medium',title:low+' product(s) low on stock',detail:'Below reorder point.'}); recs.push('Reorder '+low+' low-stock product(s) before they sell out.'); }
      if (ads>0 && mer<2){ score-=12; findings.push({severity:'high',title:'MER is '+mer.toFixed(1)+'x',detail:'Marketing efficiency under 2x — ads may be unprofitable.'}); recs.push('Pause or fix campaigns dragging MER below 2x.'); }
      if (refundRate>5){ score-=10; findings.push({severity:'medium',title:'Refund rate '+refundRate.toFixed(1)+'%',detail:'Above the healthy 5% threshold.'}); recs.push('Investigate refund drivers (product, shipping, expectations).'); }
      if (marginPct>0 && marginPct<30){ score-=8; findings.push({severity:'medium',title:'Gross margin '+marginPct.toFixed(0)+'%',detail:'Thin margin limits ad headroom.'}); recs.push('Review pricing or COGS to lift gross margin above 30%.'); }
      score=Math.max(0,Math.min(100,score));
      const status = score>=80?'good':score>=60?'warning':'critical';
      const summary = 'Store did $'+Math.round(rev).toLocaleString()+' revenue / $'+Math.round(+m.profit).toLocaleString()+' net over 30 days at '+marginPct.toFixed(0)+'% margin'+(ads>0?', MER '+mer.toFixed(1)+'x':'')+'. '+(findings.length?findings.length+' issue(s) to address.':'No major issues.');
      return { available:true, score, status, summary, findings, recommendations: recs.length?recs:['On track — no urgent fixes.'], auditedAt:new Date().toISOString() };
    }
    const creds = await getCreds('hub_' + product.id);
    const url = creds.url || product.defaultUrl;
    const apiKey = creds.api_key || process.env['HUB_' + product.id.toUpperCase() + '_KEY'];
    if (!url) return { available:false, error:'URL not configured' };
    const resp = await axios.get(url.replace(/\/+$/,'') + '/api/hub/audit', { headers:{ 'x-hub-key':apiKey, 'Authorization':'Bearer '+apiKey }, timeout:15000 }).catch(e=>({data:null,error:e.message}));
    if (!resp.data) return { available:false, error:resp.error||'No response' };
    return { available:true, ...resp.data };
  } catch(e){ return { available:false, error:e.message }; }
}

// Read the single-row CEO report config (with safe defaults)
async function getCeoConfig() {
  try {
    const r = await pool.query("SELECT * FROM ceo_report_config WHERE id=1");
    if (r.rows[0]) {
      const c = r.rows[0];
      const rec = Array.isArray(c.recipients) ? c.recipients : (typeof c.recipients==='string' ? JSON.parse(c.recipients||'[]') : []);
      return { recipients: rec, daily_enabled: c.daily_enabled!==false, weekly_enabled: c.weekly_enabled!==false, meta_daily_enabled: c.meta_daily_enabled!==false, daily_time: c.daily_time||'08:00', weekly_time: c.weekly_time||'08:00', audit_time: c.audit_time||'14:00', timezone: c.timezone||'America/New_York' };
    }
  } catch(e){}
  return { recipients:[], daily_enabled:true, weekly_enabled:true, meta_daily_enabled:true, daily_time:'08:00', weekly_time:'08:00', audit_time:'14:00', timezone:'America/New_York' };
}

// Run audits across all products and store them
async function runAuditAll() {
  const results = [];
  for (const p of HUB_PRODUCTS) {
    const a = await fetchProductAudit(p);
    if (a.available) {
      try { await pool.query("INSERT INTO hub_audit_log(product_id,product_name,score,status,summary,findings,recommendations) VALUES($1,$2,$3,$4,$5,$6,$7)", [p.id, p.name, (a.score!=null?Math.round(+a.score):null), a.status||null, a.summary||null, JSON.stringify(a.findings||[]), JSON.stringify(a.recommendations||[])]); } catch(e){ console.error('audit save '+p.id+':', e.message); }
      results.push({ id:p.id, name:p.name, icon:p.icon, ...a });
    } else {
      results.push({ id:p.id, name:p.name, icon:p.icon, available:false, error:a.error });
    }
  }
  console.log('🔍 Audit-all complete:', results.filter(r=>r.available).length+'/'+HUB_PRODUCTS.length+' products');
  return results;
}

// Build the CEO briefing email (numbers we compute + AI narrative). period = 'daily' | 'weekly'
async function generateCeoReport(period) {
  const isWeekly = period === 'weekly';
  const curRange  = isWeekly ? "date >= CURRENT_DATE - INTERVAL '7 days'" : "date = CURRENT_DATE - INTERVAL '1 day'";
  const prevRange = isWeekly ? "date >= CURRENT_DATE - INTERVAL '14 days' AND date < CURRENT_DATE - INTERVAL '7 days'" : "date = CURRENT_DATE - INTERVAL '2 days'";
  const sel = "COALESCE(SUM(revenue),0) rev, COALESCE(SUM(net_profit),0) profit, COALESCE(SUM(gross_profit),0) gp, COALESCE(SUM(orders_count),0) orders, COALESCE(SUM(ad_spend),0) ads, COALESCE(SUM(new_customers),0) newc, COALESCE(SUM(refund_total),0) refunds, COALESCE(AVG(NULLIF(aov,0)),0) aov";
  const cur  = (await pool.query(`SELECT ${sel} FROM daily_metrics WHERE ${curRange}`)).rows[0];
  const prev = (await pool.query(`SELECT ${sel} FROM daily_metrics WHERE ${prevRange}`)).rows[0];
  let freshNote = '';
  try {
    const latest = (await pool.query("SELECT MAX(date) d FROM daily_metrics")).rows[0]?.d;
    if (!isWeekly && +cur.orders===0 && +cur.rev===0) freshNote = "No sales recorded for yesterday yet — figures may still be syncing.";
    else if (latest) { const ld = new Date(latest); const ageDays = Math.floor((Date.now()-ld.getTime())/864e5); if (ageDays >= 2) freshNote = "Latest data is from " + ld.toLocaleDateString('en-US',{month:'short',day:'numeric'}) + " — the store sync may be behind."; }
  } catch {}
  const products = [];
  for (const p of HUB_PRODUCTS) {
    if (p.id === 'tvs_store') continue;
    const s = await fetchProductStats(p);
    let audit = null;
    if (isWeekly) { try { audit = (await pool.query("SELECT score,status,summary FROM hub_audit_log WHERE product_id=$1 ORDER BY audited_at DESC LIMIT 1",[p.id])).rows[0]||null; } catch {} }
    products.push({ id:p.id, name:p.name, icon:p.icon, connected:!!s.connected, metrics:s.metrics||{}, error:s.error, audit });
  }
  const fc = v => '$' + Math.round(+v||0).toLocaleString('en-US');
  const pctv = (c,p) => { c=+c; p=+p; if(!p) return null; return (c-p)/Math.abs(p)*100; };
  const chip = (c,p,invert) => { const v=pctv(c,p); if(v===null) return ''; const good = invert? v<0 : v>0; const col = Math.abs(v)<0.5?'#64748b':(good?'#16a34a':'#dc2626'); const ar = v>0.5?'▲':v<-0.5?'▼':''; return `<span style="color:${col};font-size:12px;font-weight:600">${ar} ${Math.abs(v).toFixed(0)}%</span>`; };
  const mer = +cur.ads>0 ? +cur.rev/+cur.ads : 0;
  const row = (label,c,p,fmt,invert) => `<tr><td style="padding:9px 0;color:#64748b;font-size:13px">${label}</td><td style="padding:9px 0;text-align:right;font-weight:700;color:#0f172a;font-size:15px">${fmt(c)}</td><td style="padding:9px 0;text-align:right">${chip(c,p,invert)}</td></tr>`;
  const num = v => Math.round(+v||0).toLocaleString('en-US');
  const metricsHtml = `<table style="width:100%;border-collapse:collapse">
    ${row('Revenue',cur.rev,prev.rev,fc)}
    ${row('Net Profit',cur.profit,prev.profit,fc)}
    ${row('Orders',cur.orders,prev.orders,num)}
    ${row('New Customers',cur.newc,prev.newc,num)}
    ${row('Ad Spend',cur.ads,prev.ads,fc,true)}
    ${row('Refunds',cur.refunds,prev.refunds,fc,true)}
    <tr><td style="padding:9px 0;color:#64748b;font-size:13px">MER</td><td style="padding:9px 0;text-align:right;font-weight:700;color:#0f172a;font-size:15px">${mer.toFixed(1)}x</td><td></td></tr>
  </table>`;
  const prodHtml = products.map(p => {
    const dot = p.connected ? '#16a34a' : (p.error && !/not configured/i.test(p.error)) ? '#dc2626' : '#cbd5e1';
    const sc = (p.audit && p.audit.score!=null) ? ` · score ${p.audit.score}` : '';
    const detail = p.connected
      ? Object.entries(p.metrics).slice(0,3).map(([k,v]) => `${k.replace(/_/g,' ')}: ${typeof v==='number'?v.toLocaleString():v}`).join(' · ')
      : (p.error && !/not configured/i.test(p.error) ? '<span style="color:#dc2626">Not responding</span>' : 'Not configured');
    return `<tr><td style="padding:8px 0;font-size:13px"><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${dot};margin-right:8px"></span>${p.icon} <strong>${p.name}</strong>${sc}</td><td style="padding:8px 0;text-align:right;color:#64748b;font-size:11px">${detail}</td></tr>`;
  }).join('');
  const aiContext = JSON.stringify({ period, store:{ current:cur, previous:prev, mer }, products }, null, 2);
  let narrative = '';
  try {
  narrative = await claudeAnalyze(
    `You are the Chief of Staff to the CEO of The Vitamin Shots (a DTC vegan supplement brand) who also owns a portfolio of SaaS products. Write the body of a ${isWeekly?'weekly':'daily'} executive briefing email. The CEO is busy and non-technical — be clear, specific with numbers, and decisive. Structure EXACTLY as these four sections, nothing else:\n\nBOTTOM LINE: 2-3 sentences on the overall situation.\n\nWHAT'S WORKING: 2-4 short dash bullets, each with a number.\n\nWHAT NEEDS ATTENTION: 2-4 short dash bullets, most urgent first, each naming the specific problem number.\n\nDO THIS ${isWeekly?'WEEK':'TODAY'}: a numbered list of 3-5 concrete prioritized actions, highest business impact first, one line each.\n\nNo greeting, no sign-off, no text outside these sections.`,
    `Data:\n${aiContext}`
  );
  } catch (e) { console.error('CEO report AI failed:', e.message); }
  if (!narrative || /^Could not|not configured|no api key/i.test(narrative)) {
    narrative = `BOTTOM LINE: Store revenue ${fc(cur.rev)} with net profit ${fc(cur.profit)} over the period.\n\nWHAT NEEDS ATTENTION:\n- Connect ANTHROPIC_API_KEY to enable full AI analysis and recommendations.`;
  }
  const narrativeHtml = narrative
    .replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')
    .replace(/(BOTTOM LINE:|WHAT'S WORKING:|WHAT NEEDS ATTENTION:|DO THIS WEEK:|DO THIS TODAY:)/g,'<h3 style="color:#0f172a;font-size:13px;text-transform:uppercase;letter-spacing:.5px;margin:18px 0 6px">$1</h3>')
    .replace(/\n\s*[-•]\s?/g,'<br>• ')
    .replace(/\n/g,'<br>');
  const label = isWeekly ? 'Weekly' : 'Daily';
  const subject = `${APP_NAME} ${label} Brief — ${fc(cur.rev)} rev · ${fc(cur.profit)} profit${(+cur.orders)?' · '+num(cur.orders)+' orders':''}`;
  const html = `<div style="font-family:'Segoe UI',Arial,sans-serif;max-width:640px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0">
    <div style="background:linear-gradient(135deg,#0f172a,#1e293b);padding:28px 32px">
      <h1 style="color:#f1c349;margin:0;font-size:22px">${label} CEO Brief</h1>
      <p style="color:#94a3b8;margin:6px 0 0;font-size:13px">The Vitamin Shots · ${new Date().toLocaleDateString('en-US',{ weekday:'long', month:'long', day:'numeric', year:'numeric' })}</p>
    </div>
    <div style="padding:28px 32px">
      <h2 style="color:#0f172a;font-size:16px;margin:0 0 6px;border-bottom:2px solid #f1f5f9;padding-bottom:10px">Store — ${isWeekly?'Last 7 Days':'Yesterday'}</h2>
      ${freshNote?`<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:10px 12px;margin:8px 0 12px;color:#92400e;font-size:12px">⚠️ ${freshNote}</div>`:''}
      ${metricsHtml}
      ${products.length ? `<h2 style="color:#0f172a;font-size:16px;margin:26px 0 6px;border-bottom:2px solid #f1f5f9;padding-bottom:10px">Software Portfolio</h2><table style="width:100%;border-collapse:collapse">${prodHtml}</table>` : ''}
      <div style="margin-top:24px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:20px">
        <div style="margin-bottom:4px"><span style="font-size:16px">🧠</span> <strong style="color:#0f172a;font-size:13px">AI Analysis &amp; Recommendations</strong></div>
        <div style="color:#334155;font-size:14px;line-height:1.65">${narrativeHtml}</div>
      </div>
    </div>
    <div style="background:#f1f5f9;padding:18px;text-align:center;font-size:11px;color:#94a3b8">Generated by ${APP_NAME} · Command Hub</div>
  </div>`;
  return { subject, html, metrics: cur };
}

// Send the CEO report to configured recipients
async function sendCeoReport(period, cfg) {
  cfg = cfg || await getCeoConfig();
  const recipients = (cfg.recipients||[]).filter(Boolean);
  if (!recipients.length) { console.log('CEO '+period+' report: no recipient configured, skipping'); return { skipped:true, reason:'no recipient' }; }
  if (!process.env.RESEND_API_KEY && !process.env.SMTP_HOST) { console.log('CEO '+period+' report: no email provider'); return { skipped:true, reason:'no email provider' }; }
  const { subject, html } = await generateCeoReport(period);
  let sent=0, lastError=null;
  for (const to of recipients) { const r = await sendEmail({ to, subject, html }); if (r?.success) sent++; else lastError = r?.error || 'unknown'; }
  console.log(`✅ CEO ${period} report sent to ${sent}/${recipients.length}`);
  return { sent, total: recipients.length, lastError };
}

// Run all audits now (also used by the Friday cron)
app.post("/api/hub/audit-all", auth, async (req, res) => {
  try { const results = await runAuditAll(); res.json({ audited: results.filter(r=>r.available).length, total: results.length, results, ranAt: new Date().toISOString() }); }
  catch(e){ res.status(500).json({ error: e.message }); }
});

// Audit history (for charts) + latest audit per product
app.get("/api/hub/audit-history", auth, async (req, res) => {
  try {
    const history = (await pool.query("SELECT product_id,product_name,score,status,audited_at FROM hub_audit_log WHERE audited_at>=CURRENT_DATE-INTERVAL '120 days' ORDER BY audited_at ASC")).rows;
    const latest = (await pool.query("SELECT DISTINCT ON (product_id) product_id,product_name,score,status,summary,findings,recommendations,audited_at FROM hub_audit_log ORDER BY product_id,audited_at DESC")).rows;
    res.json({ history, latest });
  } catch(e){ res.status(500).json({ error: e.message }); }
});

// Ask AI about ALL businesses at once (combines live data across every product)
app.post("/api/hub/ask", auth, async (req, res) => {
  try {
    ...
    for (const p of HUB_PRODUCTS) {
      if (p.id === 'tvs_store') {
        const m = (await pool.query("SELECT COALESCE(SUM(revenue),0) rev, COALESCE(SUM(net_profit),0) profit, COALESCE(SUM(orders_count),0) orders, COALESCE(SUM(ad_spend),0) ads, COALESCE(SUM(new_customers),0) newc FROM daily_metrics WHERE date>=CURRENT_DATE-INTERVAL '30 days'")).rows[0];
        products.push({ name:'TVS Store', connected:true, metrics:{ revenue_30d:Math.round(+m.rev), net_profit_30d:Math.round(+m.profit), orders_30d:+m.orders, new_customers_30d:+m.newc, ad_spend_30d:Math.round(+m.ads) } });
        continue;
      }
      const s = await fetchProductStats(p);
      products.push({ name:p.name, connected:!!s.connected, metrics:s.metrics||{}, error:s.error });
    }
    const answer = await claudeAnalyze(
      "You are the Chief of Staff to the CEO of The Vitamin Shots, with live visibility into every one of their software products and the store. Answer the CEO's question using this data. Combine insights across products where relevant. Be specific with numbers and end with a clear recommendation. Keep it under 250 words.",
      "Live data across all products:\n" + JSON.stringify({ products }, null, 2) + "\n\nCEO question: " + question
    );
    res.json({ answer, generatedAt: new Date().toISOString() });
  } catch(e){ res.status(500).json({ error: e.message }); }
});

// CEO report config — get / update
app.get("/api/settings/ceo-report", auth, async (req, res) => {
  try { res.json(await getCeoConfig()); } catch(e){ res.status(500).json({ error: e.message }); }
});
app.put("/api/settings/ceo-report", auth, async (req, res) => {
  try {
    const { recipients, daily_enabled, weekly_enabled, meta_daily_enabled, daily_time, weekly_time, audit_time, timezone } = req.body;
    const recArr = Array.isArray(recipients) ? recipients.map(s=>String(s).trim().toLowerCase()).filter(Boolean)
      : (typeof recipients==='string' ? recipients.split(',').map(s=>s.trim().toLowerCase()).filter(Boolean) : null);
    await pool.query(`INSERT INTO ceo_report_config(id,recipients,daily_enabled,weekly_enabled,meta_daily_enabled,daily_time,weekly_time,audit_time,timezone,updated_at)
      VALUES(1,$1,$2,$3,$4,$5,$6,$7,$8,NOW())
      ON CONFLICT(id) DO UPDATE SET recipients=COALESCE($1,ceo_report_config.recipients),daily_enabled=COALESCE($2,ceo_report_config.daily_enabled),weekly_enabled=COALESCE($3,ceo_report_config.weekly_enabled),meta_daily_enabled=COALESCE($4,ceo_report_config.meta_daily_enabled),daily_time=COALESCE($5,ceo_report_config.daily_time),weekly_time=COALESCE($6,ceo_report_config.weekly_time),audit_time=COALESCE($7,ceo_report_config.audit_time),timezone=COALESCE($8,ceo_report_config.timezone),updated_at=NOW()`,
      [ recArr!=null?JSON.stringify(recArr):null, daily_enabled, weekly_enabled, meta_daily_enabled, daily_time||null, weekly_time||null, audit_time||null, timezone||null ]);
    res.json(await getCeoConfig());
  } catch(e){ res.status(500).json({ error: e.message }); }
});

// Manually send a CEO report now (period: daily | weekly)
app.post("/api/hub/send-report", auth, async (req, res) => {
  try {
    const raw = String(req.body.period || 'daily');
    const isMeta = raw.startsWith('meta');
    const period = raw.includes('weekly') ? 'weekly' : 'daily';
    const r = isMeta ? await sendMetaReport(period) : await sendCeoReport(period);
    if (r.skipped) return res.status(400).json({ error: r.reason==='no recipient' ? 'No CEO recipient set. Add an email in Settings → CEO Report.' : 'No email provider configured.' });
    if (!r.sent) return res.status(500).json({ error: 'Report generated but delivery failed. ' + (r.lastError||'') });
    res.json({ sent: r.sent, period: raw });
  } catch(e){ res.status(500).json({ error: e.message }); }
});

// Per-product detailed analytics (rich: kpis/series/breakdowns/comparisons) with range filter
const RANGE_DAYS = { '24h':1, '7d':7, '30d':30, '90d':90, 'all':3650 };
app.get("/api/hub/product/:id/analytics", auth, async (req, res) => {
  try {
    const id = req.params.id;
    const range = RANGE_DAYS[req.query.range] ? req.query.range : '30d';
    const days = RANGE_DAYS[range];
    const product = HUB_PRODUCTS.find(p => p.id === id);
    if (!product) return res.status(404).json({ error: 'Unknown product' });

    if (id === 'tvs_store') {
      const since = `CURRENT_DATE - INTERVAL '${days} days'`;
      const prevSince = `CURRENT_DATE - INTERVAL '${days*2} days'`;
      const rows = (await pool.query(`SELECT date, COALESCE(revenue,0) revenue, COALESCE(net_profit,0) net_profit, COALESCE(orders_count,0) orders, COALESCE(ad_spend,0) ad_spend, COALESCE(meta_spend,0) meta_spend, COALESCE(google_spend,0) google_spend, COALESCE(tiktok_spend,0) tiktok_spend, COALESCE(organic_revenue,0) organic_revenue, COALESCE(email_revenue,0) email_revenue FROM daily_metrics WHERE date >= ${since} ORDER BY date ASC`)).rows;
      const cur  = (await pool.query(`SELECT COALESCE(SUM(revenue),0) rev, COALESCE(SUM(net_profit),0) profit, COALESCE(SUM(orders_count),0) orders, COALESCE(SUM(ad_spend),0) ads, COALESCE(SUM(new_customers),0) newc, COALESCE(AVG(NULLIF(aov,0)),0) aov FROM daily_metrics WHERE date >= ${since}`)).rows[0];
      const prev = (await pool.query(`SELECT COALESCE(SUM(revenue),0) rev, COALESCE(SUM(net_profit),0) profit, COALESCE(SUM(orders_count),0) orders, COALESCE(SUM(ad_spend),0) ads, COALESCE(SUM(new_customers),0) newc FROM daily_metrics WHERE date >= ${prevSince} AND date < ${since}`)).rows[0];
      const ch = (c,p)=>{ c=+c;p=+p; return p? +(((c-p)/Math.abs(p))*100).toFixed(1) : null; };
      const mer = +cur.ads>0 ? +cur.rev/+cur.ads : 0, prevMer = +prev.ads>0 ? +prev.rev/+prev.ads : 0;
      const sum = (k)=> rows.reduce((a,r)=>a+ +r[k],0);
      const kpis = [
        { key:'revenue', label:'Revenue', value:Math.round(+cur.rev), unit:'$', changePct: ch(cur.rev,prev.rev) },
        { key:'net_profit', label:'Net Profit', value:Math.round(+cur.profit), unit:'$', changePct: ch(cur.profit,prev.profit) },
        { key:'orders', label:'Orders', value:+cur.orders, unit:'', changePct: ch(cur.orders,prev.orders) },
        { key:'new_customers', label:'New Customers', value:+cur.newc, unit:'', changePct: ch(cur.newc,prev.newc) },
        { key:'aov', label:'AOV', value:Math.round(+cur.aov), unit:'$', changePct: null },
        { key:'mer', label:'MER', value:+mer.toFixed(2), unit:'x', changePct: ch(mer,prevMer) },
      ];
      const series = [
        { name:'Revenue', color:'#f1c349', points: rows.map(r=>({ x:r.date, y:Math.round(+r.revenue) })) },
        { name:'Net Profit', color:'#16a34a', points: rows.map(r=>({ x:r.date, y:Math.round(+r.net_profit) })) },
        { name:'Orders', color:'#6366f1', points: rows.map(r=>({ x:r.date, y:+r.orders })) },
        { name:'Ad Spend', color:'#ef4444', points: rows.map(r=>({ x:r.date, y:Math.round(+r.ad_spend) })) },
      ];
      const otherRev = Math.max(0, sum('revenue') - sum('organic_revenue') - sum('email_revenue'));
      const breakdowns = [
        { title:'Revenue by source', chart:'pie', data:[ {label:'Organic',value:Math.round(sum('organic_revenue'))}, {label:'Email',value:Math.round(sum('email_revenue'))}, {label:'Other',value:Math.round(otherRev)} ].filter(d=>d.value>0) },
        { title:'Ad spend by platform', chart:'bar', data:[ {label:'Meta',value:Math.round(sum('meta_spend'))}, {label:'Google',value:Math.round(sum('google_spend'))}, {label:'TikTok',value:Math.round(sum('tiktok_spend'))} ].filter(d=>d.value>0) },
      ];
      const comparisons = [
        { title:'This period vs previous', unit:'$', items:[ {label:'Revenue',value:Math.round(+cur.rev),highlight:true}, {label:'Revenue (prev)',value:Math.round(+prev.rev)}, {label:'Profit',value:Math.round(+cur.profit),highlight:true}, {label:'Profit (prev)',value:Math.round(+prev.profit)} ] },
      ];
      return res.json({ product:{ id, name:product.name, icon:product.icon }, connected:true, range, generatedAt:new Date().toISOString(), kpis, series, breakdowns, comparisons, recentActivity: [] });
    }

    const creds = await getCreds('hub_' + id);
    const url = creds.url || product.defaultUrl;
    const apiKey = creds.api_key || process.env['HUB_' + id.toUpperCase() + '_KEY'];
    if (!url) return res.json({ product:{ id, name:product.name, icon:product.icon }, connected:false, error:'Not configured', range });
    const resp = await axios.get(url.replace(/\/+$/,'') + '/api/hub/stats?range=' + range, { headers:{ 'x-hub-key':apiKey, 'Authorization':'Bearer '+apiKey }, timeout:15000 }).catch(e=>({ data:null, error:e.message }));
    if (!resp.data) return res.json({ product:{ id, name:product.name, icon:product.icon }, connected:false, error: resp.error||'No response', range });
    const d = resp.data;
    let kpis = Array.isArray(d.kpis) ? d.kpis : null;
    if (!kpis && d.metrics && typeof d.metrics === 'object') kpis = Object.entries(d.metrics).map(([k,v])=>({ key:k, label:k.replace(/_/g,' '), value:v, unit: /revenue|amount|spend|ltv|profit/.test(k)?'$':(/rate|pct/.test(k)?'%':''), changePct:null }));
    return res.json({ product:{ id, name:product.name, icon:product.icon }, connected:true, range, generatedAt: d.generatedAt||new Date().toISOString(), kpis: kpis||[], series: d.series||[], breakdowns: d.breakdowns||[], comparisons: d.comparisons||[], recentActivity: d.recentActivity||[] });
  } catch(e){ res.status(500).json({ error: e.message }); }
});

// ====================== META ADS (dedicated) ======================
app.get("/api/ads/meta", auth, async (req, res) => {
  try {
    const days = Math.min(180, Math.max(1, +req.query.days || 30));
    const since = `CURRENT_DATE - INTERVAL '${days} days'`;
    const daily = (await pool.query(`SELECT date, COALESCE(SUM(spend),0) spend, COALESCE(SUM(impressions),0) impressions, COALESCE(SUM(clicks),0) clicks, COALESCE(SUM(conversions),0) conversions, COALESCE(SUM(conversion_value),0) revenue FROM ad_spend_daily WHERE platform='meta' AND date>=${since} GROUP BY date ORDER BY date ASC`)).rows;
    const campaigns = (await pool.query(`SELECT campaign_name, COALESCE(SUM(spend),0) spend, COALESCE(SUM(impressions),0) impressions, COALESCE(SUM(clicks),0) clicks, COALESCE(SUM(conversions),0) conversions, COALESCE(SUM(conversion_value),0) revenue FROM ad_spend_daily WHERE platform='meta' AND date>=${since} GROUP BY campaign_name ORDER BY spend DESC LIMIT 50`)).rows;
    const t = daily.reduce((a,r)=>({ spend:a.spend+ +r.spend, rev:a.rev+ +r.revenue, clicks:a.clicks+ +r.clicks, imp:a.imp+ +r.impressions, conv:a.conv+ +r.conversions }), { spend:0, rev:0, clicks:0, imp:0, conv:0 });
    res.json({ days, totals: { spend:t.spend, revenue:t.rev, roas: t.spend>0?t.rev/t.spend:0, cpa: t.conv>0?t.spend/t.conv:0, cpc: t.clicks>0?t.spend/t.clicks:0, ctr: t.imp>0?t.clicks/t.imp*100:0, clicks:t.clicks, impressions:t.imp, conversions:t.conv }, daily, campaigns });
  } catch(e){ res.status(500).json({ error: e.message }); }
});
app.post("/api/ads/meta/ai", auth, async (req, res) => {
  try {
    const since = "CURRENT_DATE - INTERVAL '30 days'";
    const campaigns = (await pool.query(`SELECT campaign_name, COALESCE(SUM(spend),0) spend, COALESCE(SUM(conversion_value),0) revenue, COALESCE(SUM(conversions),0) conversions, COALESCE(SUM(clicks),0) clicks, COALESCE(SUM(impressions),0) impressions FROM ad_spend_daily WHERE platform='meta' AND date>=${since} GROUP BY campaign_name ORDER BY spend DESC LIMIT 25`)).rows;
    const answer = await claudeAnalyze(
      "You are a senior Meta Ads strategist for The Vitamin Shots (DTC vegan supplements). Analyze the campaign data and give: 1) an overall verdict on Meta performance, 2) which campaigns to scale, 3) which to cut or fix, 4) three concrete next actions. Be specific with ROAS and CPA numbers. Under 250 words, use clear bullets.",
      "Meta campaigns (last 30 days):\n" + JSON.stringify(campaigns, null, 2)
    );
    res.json({ answer, generatedAt: new Date().toISOString() });
  } catch(e){ res.status(500).json({ error: e.message }); }
});

// ---- Dedicated Meta Ads report (separate daily email to the CEO) ----
async function generateMetaReport(period) {
  const isWeekly = period==='weekly';
  const curRange  = isWeekly ? "date >= CURRENT_DATE - INTERVAL '7 days'" : "date = CURRENT_DATE - INTERVAL '1 day'";
  const prevRange = isWeekly ? "date >= CURRENT_DATE - INTERVAL '14 days' AND date < CURRENT_DATE - INTERVAL '7 days'" : "date = CURRENT_DATE - INTERVAL '2 days'";
  const sel = "COALESCE(SUM(spend),0) spend, COALESCE(SUM(conversion_value),0) revenue, COALESCE(SUM(conversions),0) conv, COALESCE(SUM(clicks),0) clicks, COALESCE(SUM(impressions),0) impr FROM ad_spend_daily WHERE platform='meta' AND ";
  const cur  = (await pool.query("SELECT "+sel+curRange)).rows[0];
  const prev = (await pool.query("SELECT "+sel+prevRange)).rows[0];
  const campaigns = (await pool.query("SELECT campaign_name, COALESCE(SUM(spend),0) spend, COALESCE(SUM(conversion_value),0) revenue FROM ad_spend_daily WHERE platform='meta' AND "+curRange+" GROUP BY campaign_name ORDER BY spend DESC LIMIT 8")).rows;
  const fc=v=>'$'+Math.round(+v||0).toLocaleString('en-US');
  const roas = +cur.spend>0 ? +cur.revenue/+cur.spend : 0, cpa = +cur.conv>0 ? +cur.spend/+cur.conv : 0, ctr = +cur.impr>0 ? +cur.clicks/+cur.impr*100 : 0;
  const prevRoas = +prev.spend>0 ? +prev.revenue/+prev.spend : 0;
  const hasData = +cur.spend>0 || +cur.revenue>0;
  const chip=(c,p)=>{ c=+c;p=+p; if(!p) return ''; const v=(c-p)/Math.abs(p)*100; const col=Math.abs(v)<0.5?'#64748b':(v>0?'#16a34a':'#dc2626'); return `<span style="color:${col};font-size:12px">${v>0?'▲':'▼'} ${Math.abs(v).toFixed(0)}%</span>`; };
  const campRows = campaigns.map(c=>{ const r=+c.spend>0?+c.revenue/+c.spend:0; return `<tr><td style="padding:7px 0;font-size:13px">${(c.campaign_name||'Unnamed')}</td><td style="padding:7px 0;text-align:right">${fc(c.spend)}</td><td style="padding:7px 0;text-align:right;font-weight:700;color:${r>=2?'#16a34a':r>0?'#d97706':'#94a3b8'}">${r?r.toFixed(1)+'x':'—'}</td></tr>`; }).join('');
  let narrative='';
  try { narrative = await claudeAnalyze(
    `You are a senior Meta Ads strategist for The Vitamin Shots. Write a short ${isWeekly?'weekly':'daily'} Meta Ads briefing for the CEO. Use EXACTLY these sections and nothing else:\n\nVERDICT: 1-2 sentences on Meta performance.\n\nSCALE: which campaigns to scale (name the ROAS).\n\nFIX/CUT: which to fix or cut (name the bad number).\n\nDO ${isWeekly?'THIS WEEK':'TODAY'}: 2-3 numbered actions. No greeting, no sign-off.`,
    "Meta data:\n"+JSON.stringify({period,current:cur,previous:prev,roas,cpa,ctr,campaigns},null,2)); } catch(e){ console.error('Meta report AI:',e.message); }
  if(!narrative || /^Could not|not configured|no api key/i.test(narrative)) narrative = hasData ? `VERDICT: ${fc(cur.spend)} spend produced ${fc(cur.revenue)} revenue at ${roas.toFixed(1)}x ROAS.` : `VERDICT: No Meta ad spend recorded for this period yet.`;
  const narrativeHtml = narrative.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/(VERDICT:|SCALE:|FIX\/CUT:|DO THIS WEEK:|DO TODAY:)/g,'<h3 style="color:#0f172a;font-size:13px;text-transform:uppercase;letter-spacing:.5px;margin:16px 0 6px">$1</h3>').replace(/\n\s*[-•]\s?/g,'<br>• ').replace(/\n/g,'<br>');
  const label = isWeekly?'Weekly':'Daily';
  const subject = `${APP_NAME} — Meta Ads ${label}: ${fc(cur.spend)} spend · ${roas.toFixed(1)}x ROAS`;
  const html = `<div style="font-family:'Segoe UI',Arial,sans-serif;max-width:620px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0">
    <div style="background:linear-gradient(135deg,#1877F2,#0a3d8f);padding:26px 30px"><h1 style="color:#fff;margin:0;font-size:21px">Meta Ads — ${label} Report</h1><p style="color:#cfe0ff;margin:5px 0 0;font-size:13px">${new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'})}</p></div>
    <div style="padding:24px 30px">
      <table style="width:100%;border-collapse:collapse">
        <tr><td style="padding:8px 0;color:#64748b;font-size:13px">Spend</td><td style="padding:8px 0;text-align:right;font-weight:700">${fc(cur.spend)}</td><td style="padding:8px 0;text-align:right">${chip(cur.spend,prev.spend)}</td></tr>
        <tr><td style="padding:8px 0;color:#64748b;font-size:13px">Revenue</td><td style="padding:8px 0;text-align:right;font-weight:700;color:#16a34a">${fc(cur.revenue)}</td><td style="padding:8px 0;text-align:right">${chip(cur.revenue,prev.revenue)}</td></tr>
        <tr><td style="padding:8px 0;color:#64748b;font-size:13px">ROAS</td><td style="padding:8px 0;text-align:right;font-weight:700;color:${roas>=2?'#16a34a':'#dc2626'}">${roas.toFixed(1)}x</td><td style="padding:8px 0;text-align:right">${chip(roas,prevRoas)}</td></tr>
        <tr><td style="padding:8px 0;color:#64748b;font-size:13px">CPA</td><td style="padding:8px 0;text-align:right;font-weight:700">${fc(cpa)}</td><td></td></tr>
        <tr><td style="padding:8px 0;color:#64748b;font-size:13px">CTR</td><td style="padding:8px 0;text-align:right;font-weight:700">${ctr.toFixed(2)}%</td><td></td></tr>
      </table>
      ${campRows?`<h2 style="color:#0f172a;font-size:15px;margin:22px 0 6px;border-bottom:2px solid #f1f5f9;padding-bottom:8px">Top Campaigns</h2><table style="width:100%;border-collapse:collapse"><tr style="font-size:10px;color:#94a3b8;text-transform:uppercase"><td>Campaign</td><td style="text-align:right">Spend</td><td style="text-align:right">ROAS</td></tr>${campRows}</table>`:''}
      <div style="margin-top:20px;background:#f0f6ff;border:1px solid #cfe0ff;border-radius:12px;padding:18px"><div style="margin-bottom:4px">📘 <strong style="color:#0f172a;font-size:13px">AI Analysis</strong></div><div style="color:#334155;font-size:14px;line-height:1.6">${narrativeHtml}</div></div>
    </div>
    <div style="background:#f1f5f9;padding:16px;text-align:center;font-size:11px;color:#94a3b8">${APP_NAME} · Meta Ads</div>
  </div>`;
  return { subject, html, hasData };
}
async function sendMetaReport(period, cfg) {
  cfg = cfg || await getCeoConfig();
  const recipients = (cfg.recipients||[]).filter(Boolean);
  if (!recipients.length) return { skipped:true, reason:'no recipient' };
  if (!process.env.RESEND_API_KEY && !process.env.SMTP_HOST) return { skipped:true, reason:'no email provider' };
  const { subject, html } = await generateMetaReport(period);
  let sent=0, lastError=null;
  for (const to of recipients) { const r = await sendEmail({ to, subject, html }); if (r?.success) sent++; else lastError=r?.error; }
  console.log(`✅ Meta ${period} report sent to ${sent}/${recipients.length}`);
  return { sent, total: recipients.length, lastError };
}

// Atomically claim a cron run for `today` (prevents duplicate sends across restarts & multiple instances)
async function cronShouldRun(key, today) {
  try {
    const r = await pool.query(
      "INSERT INTO cron_state(key,last_run_date) VALUES($1,$2) ON CONFLICT(key) DO UPDATE SET last_run_date=$2 WHERE cron_state.last_run_date IS DISTINCT FROM $2 RETURNING key",
      [key, today]
    );
    return r.rowCount > 0;
  } catch (e) { console.error('cronShouldRun:', e.message); return false; }
}

// CEO Cockpit scheduler — hourly tick that honors configurable times/timezone
cron.schedule('0 * * * *', async () => {
  try {
    ...
    const auditHH  = (cfg.audit_time ||'14:00').slice(0,2);
    const dailyHH  = (cfg.daily_time ||'08:00').slice(0,2);
    const weeklyHH = (cfg.weekly_time||'08:00').slice(0,2);
    // Friday deep re-audit → caches results for Saturday's report
    if (dow===5 && hh===auditHH && await cronShouldRun('ceo_fri_audit', today)) { console.log('🔍 Friday auto-audit…'); await runAuditAll().catch(e=>console.error('Friday audit:',e.message)); }
    // Weekly report on Saturday (the comprehensive one)
    if (cfg.weekly_enabled && dow===6 && hh===weeklyHH && await cronShouldRun('ceo_weekly', today)) { await sendCeoReport('weekly',cfg).catch(e=>console.error('Weekly CEO report:',e.message)); }
    // Daily brief every day — but skip Saturday when the weekly already covers it
    if (cfg.daily_enabled && hh===dailyHH && !(dow===6 && cfg.weekly_enabled) && await cronShouldRun('ceo_daily', today)) { await sendCeoReport('daily',cfg).catch(e=>console.error('Daily CEO report:',e.message)); }
    // Dedicated daily Meta Ads email (separate from the brief)
    if (cfg.meta_daily_enabled!==false && cfg.daily_enabled && hh===dailyHH && await cronShouldRun('ceo_meta_daily', today)) { await sendMetaReport('daily',cfg).catch(e=>console.error('Daily Meta report:',e.message)); }
  } catch(e){ console.error('CEO cron:', e.message); }
...
// ====================== SMART NOTIFICATIONS ======================
app.all("/api/notifications/check", auth, async (req, res) => {
  try {
    const alerts = [];
...
async function sendWeeklyReport() {
  try {
    ...
    console.log("✅ Weekly report sent to", recipients.join(", "), "id:", info.id);
...
cron.schedule('0 8 * * 1',async()=>{console.log('📧 Sending weekly report...');
    const _wcfg = await getCeoConfig();
    if (_wcfg.weekly_enabled && (_wcfg.recipients||[]).filter(Boolean).length) { console.log('↪︎ Legacy weekly skipped — CEO weekly report is active'); return; }
    // Also generate AI weekly report if API key available
    if (process.env.ANTHROPIC_API_KEY) {
      try {
        const tw = (await pool.query("SELECT SUM(revenue) as rev, SUM(net_profit) as profit, SUM(orders_count) as orders, SUM(ad_spend) as ads FROM daily_metrics WHERE date>=CURRENT_DATE-INTERVAL '7 days'")).rows[0];
        const lw = (await pool.query("SELECT SUM(revenue) as rev, SUM(net_profit) as profit, SUM(orders_count) as orders FROM daily_metrics WHERE date>=CURRENT_DATE-INTERVAL '14 days' AND date<CURRENT_DATE-INTERVAL '7 days'")).rows[0];
        const aiReport = await claudeAnalyze('You are the AI Finance Director for The Vitamin Shots. Write a brief 200-word weekly email summary.', JSON.stringify({thisWeek:tw,lastWeek:lw}));
        // Append AI insights to the weekly email
        console.log('AI weekly insights generated');
      } catch(e) { console.log('AI report skipped:', e.message); }
    }await sendWeeklyReport()},{timezone:cronTz});
...
(async () => {
  ...
  await pool.query("ALTER TABLE fixed_costs ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true").catch(()=>{});
  ...
  const migrations = [
    ...
    "INSERT INTO integrations(platform,is_connected) VALUES('microsoft_ads',false),('google_analytics',false),('search_console',false),('referly',false),('paypal',false),('slack',false),('resend',false),('enginemailer',false),('emailit',false),('emailit_smtp',false) ON CONFLICT(platform) DO NOTHING",
    ...
    "CREATE TABLE IF NOT EXISTS ad_spend_hourly(id SERIAL PRIMARY KEY,date DATE NOT NULL,hour INTEGER NOT NULL,platform VARCHAR(50),campaign_name VARCHAR(500),spend DECIMAL(12,2) DEFAULT 0,impressions INTEGER DEFAULT 0,clicks INTEGER DEFAULT 0,conversions INTEGER DEFAULT 0,conversion_value DECIMAL(12,2) DEFAULT 0,UNIQUE(date,hour,platform,campaign_name))",
    "CREATE INDEX IF NOT EXISTS idx_ad_hourly_date ON ad_spend_hourly(date,hour)",
    ...
    "CREATE TABLE IF NOT EXISTS hub_audit_log(id SERIAL PRIMARY KEY,product_id VARCHAR(100),product_name VARCHAR(255),score INTEGER,status VARCHAR(50),summary TEXT,findings JSONB DEFAULT '[]',recommendations JSONB DEFAULT '[]',audited_at TIMESTAMPTZ DEFAULT NOW())",
    'CREATE INDEX IF NOT EXISTS idx_hub_audit_product ON hub_audit_log(product_id,audited_at)',
    "CREATE TABLE IF NOT EXISTS ceo_report_config(id INTEGER PRIMARY KEY,recipients JSONB DEFAULT '[]',daily_enabled BOOLEAN DEFAULT true,weekly_enabled BOOLEAN DEFAULT true,daily_time VARCHAR(5) DEFAULT '08:00',weekly_time VARCHAR(5) DEFAULT '08:00',audit_time VARCHAR(5) DEFAULT '14:00',timezone VARCHAR(64) DEFAULT 'America/New_York',updated_at TIMESTAMPTZ DEFAULT NOW())",
    "INSERT INTO ceo_report_config(id) VALUES(1) ON CONFLICT(id) DO NOTHING",
    "ALTER TABLE ceo_report_config ADD COLUMN IF NOT EXISTS meta_daily_enabled BOOLEAN DEFAULT true",
    "CREATE TABLE IF NOT EXISTS otp_codes(key VARCHAR(255) PRIMARY KEY, code VARCHAR(12) NOT NULL, expires_at TIMESTAMPTZ NOT NULL)",
    "CREATE TABLE IF NOT EXISTS cron_state(key VARCHAR(100) PRIMARY KEY, last_run_date VARCHAR(10))",
  ];
  try {
    ...
    // Hub product connections
    await pool.query("INSERT INTO integrations(platform,config,is_connected) VALUES('hub_botdolphin','{}',false) ON CONFLICT(platform) DO NOTHING");
    await pool.query("INSERT INTO integrations(platform,config,is_connected) VALUES('hub_webcomplifier','{}',false) ON CONFLICT(platform) DO NOTHING");
    await pool.query("INSERT INTO integrations(platform,config,is_connected) VALUES('hub_sitescannify','{}',false) ON CONFLICT(platform) DO NOTHING");
    await pool.query("INSERT INTO integrations(platform,config,is_connected) VALUES('hub_audience_iq','{}',false) ON CONFLICT(platform) DO NOTHING");
    await pool.query("INSERT INTO integrations(platform,config,is_connected) VALUES('hub_zerorank','{}',false) ON CONFLICT(platform) DO NOTHING");
    await pool.query("INSERT INTO integrations(platform,config,is_connected) VALUES('hub_gsc_reporter','{}',false) ON CONFLICT(platform) DO NOTHING");
    await pool.query("INSERT INTO integrations(platform,config,is_connected) VALUES('hub_amazon_tool','{}',false) ON CONFLICT(platform) DO NOTHING");
    await pool.query("INSERT INTO integrations(platform,config,is_connected) VALUES('hub_dispute_shield','{}',false) ON CONFLICT(platform) DO NOTHING");
    await pool.query("INSERT INTO integrations(platform,config,is_connected) VALUES('hub_wellness_app','{}',false) ON CONFLICT(platform) DO NOTHING");
    await pool.query("INSERT INTO integrations(platform,config,is_connected) VALUES('hub_admin_panel','{}',false) ON CONFLICT(platform) DO NOTHING");
    for (const _hp of HUB_PRODUCTS) { if (_hp.id !== 'tvs_store') await pool.query("INSERT INTO integrations(platform,config,is_connected) VALUES($1,'{}',false) ON CONFLICT(platform) DO NOTHING",['hub_'+_hp.id]); }
    await pool.query("ALTER TABLE products ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true").catch(()=>{});
...
cron.schedule('30 8 * * *', async () => {
  ...
  try {
    ...
    const _cfg = await getCeoConfig();
    if (_cfg.daily_enabled && (_cfg.recipients||[]).filter(Boolean).length) { console.log('↪︎ Legacy daily digest skipped — CEO daily brief is active'); return; }
    const today = new Date().toISOString().split('T')[0];
    const yesterday = new Date(Date.now()-864e5).toISOString().split('T')[0];
...
// 404 for unmatched API routes (must be after all routes)
app.use('/api', (req, res) => res.status(404).json({ error: 'Not found', path: req.originalUrl }));
// Global error handler — with express-async-errors above, this also catches thrown async errors
// eslint-disable-next-line no-unused-vars
app.use((err, req, res, next) => {
  console.error('Unhandled route error:', err && err.message, '@', req.method, req.originalUrl);
